...
bool RandomItemMgr::HandleConsoleCommand(ChatHandler* handler, char const* args)
{
    if (!args || !*args)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_ERROR,  "Usage: rnditem");
        return false;
    }
...
void RandomItemMgr::BuildRandomItemCache()
{
    ...
    if (results)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "Loading random item cache");
        int count = 0;
        do
        {
        ...
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "Equipment cache loaded from %d records", count);
    }
    else
    {
        ...
        for (uint32 level = 0; level < (maxLevel / 10); level++)
        {
            for (uint32 type = RANDOM_ITEM_GUILD_TASK; type <= RANDOM_ITEM_GUILD_TASK_REWARD_TRADE_RARE; type++)
            {
                ...
                sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "    Level %d..%d Type %d - %zu random items cached",
                        level * 10, level * 10 + 9,
                        type,
                        list.size());
                for (RandomItemList::iterator i = list.begin(); i != list.end(); ++i)
                {
                    ...
                    sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "        [%d] %s", itemId, proto->Name1);
                }
            }
        }
    }
...
void RandomItemMgr::BuildItemInfoCache()
{
    ...
    // load weightscales
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "Loading weightscales info");
    ...
    if (results)
    {
        ...
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "Loaded %d weightscale class specs", totalcount);
        ...
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "Loaded %d weightscale stat weights", statcount);
    }

    if (m_weightScales[1].stats.empty())
    {
        sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "Error loading item weight scales");
        return;
    }

    // vendor items
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "Loading vendor item list...");
    ...
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "Loaded %d vendor items...", (uint32)vendorItems.size());
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "Loaded %d alliance only vendor items...", (uint32)allianceItems.size());
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "Loaded %d horde only vendor items...", (uint32)hordeItems.size());

    // calculate drop source
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "Loading loot templates...");
    ...
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "Loaded %d loot templates...", (uint32)dropMap->size());

    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "Calculating stat weights for %d items...", sObjectMgr.GetItemPrototypeMap().size());
    ...
    for (uint32 itemId = 0; itemId < sObjectMgr.GetItemPrototypeMap().size(); ++itemId)
    {
        ...
        if (proto->Flags & ITEM_FLAG_NO_DISENCHANT)
        {
            ...
            sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Item: %d, source: PvP Reward", proto->ItemId);
        }
        ...
        if (cacheInfo->source == ITEM_SOURCE_NONE || cacheInfo->source == ITEM_SOURCE_PVP)
        {
            ...
            if (questIds.size())
            {
                ...
                sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Item: %d, team (quest): %s", proto->ItemId, cacheInfo->team == ALLIANCE ? "Alliance" : cacheInfo->team == HORDE ? "Horde" : "Both");
                sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Item: %d, source: quest %d, minlevel: %d", proto->ItemId, cacheInfo->sourceIds.front(), cacheInfo->minLevel);
            }
        }

        if (cacheInfo->minLevel)
            sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Item: %d, minlevel: %d", proto->ItemId, cacheInfo->minLevel);
        ...
        if (cacheInfo->source == ITEM_SOURCE_NONE || cacheInfo->source == ITEM_SOURCE_PVP)
        {
            ...
            for (auto& vendor : GAI_VALUE2(std::list<int32>, "item vendor list", itemId))
            {
                ...
                VendorItemData const* tItems = sObjectMgr.GetNpcVendorTemplateItemList(0 /* VendorTemplateId not in vmangos */);
                ...
                if (vItems || tItems)
                {
                    ...
                    for (int i = 0; i < numitems; ++i)
                    {
                        ...
                        if (!crItem || !0 /* conditionId not in vmangos TrainerSpell */)
                            continue;

                        if (auto result = WorldDatabase.PQuery("SELECT type, value1, value2 FROM conditions WHERE condition_entry = '%u'", 0 /* conditionId not in vmangos TrainerSpell */))
                        {
                            do
                            {
                                Field *fields = result->Fetch();
            ...
            if (cacheInfo->source == ITEM_SOURCE_VENDOR)
                sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Item: %d, source: vendor", proto->ItemId);
        }

        if (cacheInfo->team)
            sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Item: %d, team (item): %s", proto->ItemId, cacheInfo->team == ALLIANCE ? "Alliance" : "Horde");
        ...
        if (cacheInfo->source == ITEM_SOURCE_NONE)
        {
            if (creatures.size())
            {
                if (creatures.size() == 1)
                {
                    ...
                    sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Item: %d, source: creature drop, ID: %d", proto->ItemId, creatures.front());
                }
                else
                {
                    cacheInfo->source = ITEM_SOURCE_DROP;
                    sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Item: %d, source: creatures drop, number: %d", proto->ItemId, (uint32)creatures.size());
                }
            }
        }
        ...
        if (cacheInfo->source == ITEM_SOURCE_NONE || (cacheInfo->source == ITEM_SOURCE_DROP && cacheInfo->sourceIds.empty()))
        {
            if (gameobjects.size())
            {
                if (gameobjects.size() == 1)
                {
                    ...
                    sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Item: %d, source: gameobject, ID: %d", proto->ItemId, gameobjects.front());
                }
                else
                {
                    cacheInfo->source = ITEM_SOURCE_DROP;
                    sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Item: %d, source: gameobjects, number: %d", proto->ItemId, (uint32)gameobjects.size());
                }
            }
        }
        ...
        for (uint8 clazz = CLASS_WARRIOR; clazz < MAX_CLASSES; ++clazz)
        {
            ...
            for (uint32 spec = 1; spec <= MAX_STAT_SCALES; ++spec)
            {
                ...
                sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Item: %d, weight: %d, class: %d, spec: %s", proto->ItemId, statW, clazz, m_weightScales[spec].info.name.c_str());
            }
        }
...
uint32 RandomItemMgr::CalculateRandomEnchantId(uint8 playerclass, uint8 spec, ItemPrototype const* proto)
{
    ...
    if (proto->RandomProperty)
    {
        ...
        if (!random_id)
        {
            sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "Enchantment id #%u used but it doesn't have records in 'ItemRandomProperties.dbc'", randomPropId);
            return 0;
        }
...
uint32 RandomItemMgr::CalculateSingleStatWeight(uint8 playerclass, uint8 spec, std::string stat, uint32 value)
{
    ...
    for (std::vector<WeightScaleStat>::iterator i = m_weightScales[spec].stats.begin(); i != m_weightScales[spec].stats.end(); ++i)
    {
        if (stat == i->stat)
        {
            ...
            if (statWeight)
                sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "stat: %s, val: %d, weight: %d, total: %d, class: %d, spec: %s", stat.c_str(), value, i->weight, statWeight, playerclass, m_weightScales[spec].info.name.c_str());
            return statWeight;
        }
    }
...
uint32 RandomItemMgr::GetUpgrade(Player* player, std::string spec, uint8 slot, uint32 quality, uint32 itemId)
{
    ...
    if (itemId && itemInfoCache[itemId])
    {
        ...
        if (oldStatWeight)
            sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "Old Item: %d, weight: %d", itemId, oldStatWeight);
        else
            sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "Old item has no stat weight");
    }
    ...
    if (closestUpgrade)
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "New Item: %d, weight: %d", closestUpgrade, closestUpgradeWeight);
...
std::vector<uint32> RandomItemMgr::GetUpgradeList(Player* player, uint32 specId, uint8 slot, uint32 quality, uint32 itemId, uint32 amount)
{
    ...
    if (listItems.size())
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "New Items: %zu, Old item:%d, New items max: %d", listItems.size(), oldStatWeight, closestUpgradeWeight);
    ...
    for (int i = 0; i < numitems; ++i)
    {
        ...
        if (crItem && crItem->item == itemId)
        {
            ...
            if (pProto)
            {
                ...
                if (0 /* conditionId not in vmangos TrainerSpell */)
                    return false;
            }
            return true;
        }
...
void RandomItemMgr::BuildEquipCache()
{
    ...
    if (results)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "Loading equipment cache for %d classes, %d levels, %d slots, %d quality from %d items",
                MAX_CLASSES, maxLevel, EQUIPMENT_SLOT_END, ITEM_QUALITY_ARTIFACT, sObjectMgr.GetItemPrototypeMap().size());
        int count = 0;
        do
        {
            Field* fields = results->Fetch();
    ...
    else
    {
        ...
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "Building equipment cache for %d classes, %d specs, %d levels, %d slots, %d quality from %d items (%zu total)",
                MAX_CLASSES, MAX_STAT_SCALES, maxLevel, EQUIPMENT_SLOT_END, ITEM_QUALITY_ARTIFACT, sObjectMgr.GetItemPrototypeMap().size(), total);
                ...
                for (uint8 slot = 0; slot < EQUIPMENT_SLOT_END; ++slot)
                {
                    for (uint32 spec = 1; spec <= MAX_STAT_SCALES; ++spec)
                    {
                        ...
                        for (uint32 quality = ITEM_QUALITY_POOR; quality <= ITEM_QUALITY_ARTIFACT; ++quality)
                        {
                            ...
                            bar.step();
                            sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Equipment cache for class: %d, level %d, slot %d, quality %d: %zu items",
        ...
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "Equipment cache saved to DB");
    }
}
...
void RandomItemMgr::BuildAmmoCache()
{
    ...
    sLog.Out(LOG_BASIC, LOG_LVL_BASIC, "Building ammo cache for %d levels", maxLevel);
	int counter1 = 0;
	...
	sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "Cached %d types of ammo", counter1); // TEST
}
...
void RandomItemMgr::BuildPotionCache()
{
    ...
    sLog.Out(LOG_BASIC, LOG_LVL_BASIC, "Building potion cache for %d levels", maxLevel);
	int counter2 = 0;
    for (uint32 level = 1; level <= maxLevel+1; level+=10)
    {
    ...
    for (uint32 level = 1; level <= maxLevel+1; level+=10)
    {
        ...
        for (int i = 0; i < 2; ++i)
        {
			...
			counter2++;
            sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Potion cache for level=%d, effect=%d: %d items", level, effect, size);
        }
    }
	sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "Cached %d types of potions", counter2); // TEST
}
...
void RandomItemMgr::BuildFoodCache()
{
    ...
    sLog.Out(LOG_BASIC, LOG_LVL_BASIC, "Building food cache for %d levels", maxLevel);
	int counter3 = 0;
    for (uint32 level = 1; level <= maxLevel+1; level+=10)
    {
    ...
    for (uint32 level = 1; level <= maxLevel+1; level+=10)
    {
        ...
        for (int i = 0; i < 2; ++i)
        {
			...
			counter3++;
            sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Food cache for level=%d, category=%d: %d items", level, category, size);
        }
    }
	sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "Cached %d types of food", counter3);
}
...
void RandomItemMgr::BuildTradeCache()
{
    ...
    sLog.Out(LOG_BASIC, LOG_LVL_BASIC, "Building trade cache for %d levels", maxLevel);
	int counter4 = 0;
    for (uint32 level = 1; level <= maxLevel+1; level+=10)
    {
        for (uint32 itemId = 0; itemId < sObjectMgr.GetItemPrototypeMap().size(); ++itemId)
    ...
    for (uint32 level = 1; level <= maxLevel+1; level+=10)
    {
        ...
        sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Trade cache for level=%d: %d items", level, size);
		counter4++;
    }
	sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "Cached %d trade categories", counter4); // TEST
}
...
void RandomItemMgr::BuildRarityCache()
{
    ...
    if (results)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_BASIC, "Loading item rarity cache");
        int count = 0;
        do
        {
            Field* fields = results->Fetch();
        ...
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "Item rarity cache loaded from %d records", count);
    }
    else
    {
        ...
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "Item rarity cache built from %u items", sObjectMgr.GetItemPrototypeMap().size());
    }
}
...
void RandomItemMgr::LoadRandomEnchantments()
{
    ...
    if (queryResult)
    {
        ...
        sLog.Out(LOG_BASIC, LOG_LVL_ERROR, ">> Loaded 0 Item Enchantment definitions. DB table `item_enchantment_template` is empty.");
