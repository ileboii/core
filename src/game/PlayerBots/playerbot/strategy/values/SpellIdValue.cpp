...
uint32 SpellIdValue::Calculate()
{
    ...
    if (ids.empty())
    {
        //sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "Please add spell %s to spell list", namepart.c_str());
    ...
    for (PlayerSpellMap::iterator itr = bot->GetSpellMap().begin(); itr != bot->GetSpellMap().end(); ++itr)
    {
        ...
        if (ids.empty())
        {
            std::string spellName = pSpellInfo->SpellName[loc];
    ...
    if (saveMana <= 1)
    {
        for (std::set<uint32>::reverse_iterator itr = spellIds.rbegin(); itr != spellIds.rend(); ++itr)
        {
            ...
            std::string spellName = sSpellMgr.GetSpellRank(pSpellInfo->Id) > 0 ? std::to_string(sSpellMgr.GetSpellRank(pSpellInfo->Id)) : "";
    ...
    else
    {
        for (std::set<uint32>::reverse_iterator i = spellIds.rbegin(); i != spellIds.rend(); ++i)
        {
            ...
            if ((sSpellMgr.GetSpellRank(*i) > sSpellMgr.GetSpellRank(highestSpellId))) highestSpellId = *i;
