...
EntryTravelPurposeMap EntryTravelPurposeMapValue::Calculate()
{
    ...
    for (uint32 entry = 0; entry < sCreatureStorage.GetMaxEntry(); ++entry)
    {
        ...
        if (cInfo->skinning_loot_id && SKILL_SKINNING == SKILL_SKINNING)
        {
    ...
    for (uint32 entry = 0; entry < 32000; ++entry)
    {
        ...
        if (0 /* flags_extra not in vmangos GameObjectInfo */ & 0x80 /* CREATURE_EXTRA_FLAG_INVISIBLE */)
            continue;
...
uint32 EntryTravelPurposeMapValue::SkillIdToGatherEntry(int32 entry)
{
    ...
    {
        GameObjectInfo const* gInfo = sObjectMgr.GetGameObjectTemplate(entry * -1);
