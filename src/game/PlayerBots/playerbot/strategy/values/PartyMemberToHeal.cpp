...
{
    uint32 damage = 0;
    /* getAttackers not available in vmangos */ if (false) if (Unit* pAttacker = (Unit*)nullptr)
...
Unit* PartyMemberToHeal::Calculate()
{
    ...
    if (!partyMembers.empty() || !needHeals.empty())
    {
        ...
        for (Player* player : partyMembers)
        {
            ...
            if (player->IsInDuelWith(nullptr) /* duel check stub */)
            {
                continue;
            }
            ...
            if (isTank && (bot->GetGroup() && bot->GetGroup()->IsMember(player->GetObjectGuid())))
            {
...
{
    return MINI_PET != 0 /* getPetType not in vmangos */;
}
...
std::vector<Player*> PartyMemberToHeal::GetPartyMembers()
{
    ...
    if (ai->HasStrategy("focus heal targets", BotState::BOT_STATE_COMBAT))
    {
        ...
        for(const ObjectGuid& focusHealTarget : focusHealTargets)
        {
            ...
            if (player && (player->GetGroup() && player->GetGroup()->IsMember(bot->GetObjectGuid())) && ai->IsSafe(player))
            {
...
Unit* PartyMemberToProtect::Calculate()
{
    ...
    for (std::list<ObjectGuid>::iterator i = attackers.begin(); i != attackers.end(); ++i)
    {
        ...
        if (unit->AI())
        {
            if (false /* IsRangedUnit not accessible */)
                isRanged = true;
        }
