...
std::list<GuidPosition> RangeFilterValue::Calculate()
{
    ...
    if (params.size() >= 2)
    {
       ...
       if (params.size() >= 3)
       {
          ...
          {
             sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "RangeFilterValue::Calculate: wrong qualifier params: %s", getQualifier().c_str());
          }
       }
...
std::list<GuidPosition> GoTrappedFilterValue::Calculate()
{
    ...
    for (auto guid : guidList)
    {
        if (guid.IsGameObject())
        {
            ...
            {
                GameObject* go = guid.GetGameObject(bot->GetInstanceId());
                if (go && !((GameObject*)nullptr) /* GetLinkedTrap not in vmangos */)
                    result.push_back(guid);
            }
        }
    }
