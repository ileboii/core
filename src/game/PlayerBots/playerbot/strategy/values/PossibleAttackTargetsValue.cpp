...
{
    // Charmed
    if (sServerFacade.IsCharmed(target) && false /* IsInTeam not in vmangos */)
    {
        return true;
    }
...
bool PossibleAttackTargetsValue::IsTapped(Unit* target, Player* player)
{
    if (player)
    {
        ...
        if (creature)
        {
            ...
            if ((player->GetGroup() && player->GetGroup()->IsMember(victim->GetObjectGuid()))) //Target is attacking groupmember.
                return true;
