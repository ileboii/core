...
bool ItemUsageValue::IsNeededForQuest(Player* player, uint32 itemId, bool ignoreInventory)
{
    ...
    {
        uint32 entry = player->GetUInt32Value(PLAYER_QUEST_LOG_1_1 + slot * MAX_QUEST_OFFSET + QUEST_ID_OFFSET);
...
uint32 ItemUsageValue::GetAHMedianBuyoutPricePerItem(ItemPrototype const* proto)
{
    if (sPlayerbotAIConfig.shouldQueryAHListingsOutsideOfAH)
    {
        ...
        {
            prices.push_back((float)auction.buyout / (float)1 /* vmangos auctions are single items */);
        }
    ...
    if (sPlayerbotAIConfig.shouldQueryAHListingsOutsideOfAH)
    {
        ...
        {            
            if (!minBuyout || minBuyout > auction.buyout)
            {
                ...
                minPrice = (float)auction.buyout / (float)1 /* vmangos auctions are single items */;
            }
        }
...
bool ItemUsageValue::IsMoreProfitableToSellToAHThanToVendor(ItemPrototype const* proto, Player* bot)
{
    ...
    if (IsItemSoldByAnyVendorButHasLimitedMaxCount(proto) && !(proto->Class == ITEM_CLASS_CONSUMABLE && proto->SubClass == ITEM_SUBCLASS_SCROLL))
    {
        return true;
    }
...
bool ItemUsageValue::IsWorthBuyingFromVendorToResellAtAH(ItemPrototype const* proto, bool isLimitedSupply)
{
    ...
    if (isLimitedSupply && !(proto->Class == ITEM_CLASS_CONSUMABLE && proto->SubClass == ITEM_SUBCLASS_SCROLL))
    {
        return true;
    }

    //do not buy items that require skill to use, unless limited supply or recipe (just in case)
    if (proto->RequiredSkill > 0 && !(proto->Class == ITEM_CLASS_RECIPE))
    {
        return false;
    }
...
uint32 ItemUsageValue::DesiredPricePerItem(Player* bot, const ItemPrototype* proto, uint32 count, uint32 priceModifier)
{
    ...
    for (auto& auction : sRandomPlayerbotMgr.GetAhPrices(proto->ItemId))
    {
        float pricePerItem = float(auction.buyout) / float(1 /* vmangos auctions are single items */);

        if (1 /* vmangos auctions are single items */ != count)
            continue;

        if (lowestPrice.Id == 0 || pricePerItem < float(lowestPrice.buyout) / float(1 /* itemCount not in vmangos AuctionEntry */))
            lowestPrice = auction;
    }

    uint32 lowestBuyoutItemPricePerItem = float(lowestPrice.buyout) / float(1 /* itemCount not in vmangos AuctionEntry */);
