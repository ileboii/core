...
bool GuildAcceptQuestOrderAction::Execute(Event& event)
{
    ...
    if (!bot->SatisfyQuestLog(false))
    {
        ...
        for (auto& [slot, questId] : candidates)
        {
            ...
            {
                QuestStatusData& qsd = *(QuestStatusData*)nullptr /* getQuestStatusMap not in vmangos */;
    ...
    for (auto& guid : npcs)
    {
        ...
        p.rpos(0);
        bot->GetSession()->HandleQuestgiverAcceptQuestOpcode(MakeTypedPacket<WorldPackets::Quest::QuestgiverAcceptQuest>(p));
