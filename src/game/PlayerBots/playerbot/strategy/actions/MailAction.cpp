    ...
    bool Process(Player* requester, int index, Mail* mail, PlayerbotAI* ai, Event& event) override
    {
        ...
        if (mail->has_items)
        {
            for (MailItemInfoVec::iterator i = mail->items.begin(); i != mail->items.end(); ++i)
            {
                ...
                ItemPrototype const *proto = sObjectMgr.GetItemPrototype(i->itemId);
                if (proto)
                {
...
public:
    ...
    bool Process(Player* requester, int index, Mail* mail, PlayerbotAI* ai, Event& event) override
    {
        ...
        if (mail->money)
        {
            ...
            bot->GetSession()->HandleMailTakeMoney(MakeTypedPacket<WorldPackets::Mail::MailTakeMoney>(packet));
        ...
        else if (mail->has_items)
        {
            ...
            for (std::list<uint32>::iterator i = guids.begin(); i != guids.end(); ++i)
            {
                ...
                bot->GetSession()->HandleMailTakeItem(MakeTypedPacket<WorldPackets::Mail::MailTakeItem>(packet));
            }
...
#endif
    bot->GetSession()->HandleMailDelete(MakeTypedPacket<WorldPackets::Mail::MailDelete>(packet));
}
