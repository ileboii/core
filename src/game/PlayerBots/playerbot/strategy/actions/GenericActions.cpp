    ...
    if (pet)
    {
        if (command == "autocast")
        {
            ...
            if (!spellName.empty())
            {
                ...
                if (pet->HasSpell(spellId) && false /* IsAutocastable not in vmangos */)
                {
        ...
        else if (command == "aggressive")
        {
            ...
            packet << data;
            packet << uint64(0);
            bot->GetSession()->HandlePetAction(MakeTypedPacket<WorldPackets::Pet::PetAction>(packet));
        ...
        else if (command == "follow")
        {
            ...
            data << command;
            data << targetGuid;
            bot->GetSession()->HandlePetAction(MakeTypedPacket<WorldPackets::Pet::PetAction>(data));
        ...
        {
            if (requester->GetMap()->GetUnit(requester->GetSelectionGuid()))
            {
                constexpr uint32 PET_IMP = 416;
                ...
                // Send pet action packet
                const ObjectGuid& petGuid = pet->GetObjectGuid();
                const ObjectGuid& targetGuid = requester->GetMap()->GetUnit(requester->GetSelectionGuid())->GetObjectGuid();
                const uint8 flag = ACT_COMMAND;
        ...
        {
            if (0 /* getPetType not in vmangos */ == HUNTER_PET)
            {
