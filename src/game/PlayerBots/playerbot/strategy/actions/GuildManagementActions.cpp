...
bool GuildManageNearbyAction::Execute(Event& event)
{
    ...
    for (auto& guid : nearGuids)
    {
        ...
        if (player->HasFlag(PLAYER_FLAGS, PLAYER_FLAGS_DND))
            continue;
        ...
        if (sPlayerbotAIConfig.inviteChat && (sRandomPlayerbotMgr.IsFreeBot(bot) || !ai->HasActivePlayerMaster()))
        {
            ...
            placeholders["%area_name"] = current_area ? current_area->Name : BOT_TEXT("string_unknown_area");
            placeholders["%zone_name"] = current_zone ? current_zone->Name : BOT_TEXT("string_unknown_area");
            ...
            for (auto line : lines)
                if (sameGroup)
                {
                    ...
                    bot->Say(line.c_str(), (bot->GetTeam() == ALLIANCE ? LANG_COMMON : LANG_ORCISH));
        }
...
bool GuildLeaveAction::Execute(Event& event)
{
    ...
    if (guild->GetMemberSize() > sPlayerbotAIConfig.guildMaxBotLimit)
    {
        ...
        guild->BroadcastToGuild(bot->GetSession(), BOT_TEXT2("I am leaving this guild to prevent it from reaching the %guild_bot_limit member limit.", placeholders).c_str(), LANG_UNIVERSAL
        );
    }
