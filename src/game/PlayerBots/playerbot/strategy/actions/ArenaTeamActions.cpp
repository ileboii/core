...
bool ArenaTeamAcceptAction::Execute(Event& event)
{
    ...
    if (accept)
    {
        ...
        sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Bot #%d <%s> accepts Arena Team invite", bot->GetGUIDLow(), bot->GetName());
        return true;
    }
    else
    {
        ...
        sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Bot #%d <%s> declines Arena Team invite", bot->GetGUIDLow(), bot->GetName());
        return false;
    }
#endif
    return false;
