...
bool EmoteActionBase::ReceiveEmote(Player* requester, Player* source, uint32 emote, bool verbal)
{
    ...
    {
        if (emoteText.size())
            bot->Say(emoteText.c_str(), (bot->GetTeam() == ALLIANCE ? LANG_COMMON : LANG_ORCISH));

        if (emoteYell.size())
            bot->Yell(emoteYell.c_str(), (bot->GetTeam() == ALLIANCE ? LANG_COMMON : LANG_ORCISH));
    }
...
bool EmoteAction::Execute(Event& event)
{
    ...
    if (!p.empty() && p.GetOpcode() == SMSG_TEXT_EMOTE)
    {
        ...
        if (namlen > 1)
            do { std::vector<char> buf(namlen); p.read((uint8*)buf.data(), namlen); nam.assign(buf.data(), namlen); } while(0);
        ...
        {
            sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Bot #%d %s:%d <%s> received SMSG_TEXT_EMOTE %d from player #%d <%s>", bot->GetGUIDLow(), bot->GetTeam() == ALLIANCE ? "A" : "H", bot->GetLevel(), bot->GetName(), text_emote, pSource->GetGUIDLow(), pSource->GetName());
            emote = text_emote;
        }
    }
    ...
    if (!p.empty() && p.GetOpcode() == SMSG_EMOTE)
    {
        ...
        if (pSource && pSource != bot && sServerFacade.GetDistance2d(bot, pSource) < sPlayerbotAIConfig.farDistance && emoteId != EMOTE_ONESHOT_NONE)
        {
            ...
            {
                sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Bot #%d %s:%d <%s> received SMSG_EMOTE %d from player #%d <%s>", bot->GetGUIDLow(), bot->GetTeam() == ALLIANCE ? "A" : "H", bot->GetLevel(), bot->GetName(), emoteId, pSource->GetGUIDLow(), pSource->GetName());
...
{
    WorldPacket p;
    bot->GetSession()->HandleMountSpecialAnimOpcode(NullClientPacket());
