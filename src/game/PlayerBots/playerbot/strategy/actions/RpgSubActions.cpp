...
void RpgHelper::resetFacing(GuidPosition guidPosition)
{
    ...
    const CreatureData* data = guidPosition.GetCreatureData();
...
bool RpgEmoteAction::Execute(Event& event)
{
    ...
    if (unit && unit->GetEntry() == 620)
    {
        ...
        data << 1;
        data << rpg->guidP();
        bot->GetSession()->HandleTextEmoteOpcode(MakeTypedPacket<WorldPackets::Misc::TextEmote>(data));
    }
    else
...
bool RpgTaxiAction::Execute(Event& event)
{
    ...
    for (uint32 i = 0; i < sTaxiPathStore.GetNumRows(); ++i)
    {
        ...
        if (entry && entry->from == node && (bot->GetTaxi().IsTaximaskNodeKnown(entry->to) || false))
        {
            nodes.push_back(i);
        }
    }
    ...
    if (nodes.empty())
    {
        sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "Bot %s - No flight paths available", bot->GetName());
        return false;
    }
    ...
    if (!flightMaster)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "Bot %s cannot talk to flightmaster (%zu location available)", bot->GetName(), nodes.size());
        return false;
    }
#ifdef MANGOSBOT_TWO                
    ...
    {
        sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "Bot %s cannot fly %u (%zu location available)", bot->GetName(), path, nodes.size());
        return false;
    }
    ...
    sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Bot #%d <%s> is flying from %s to %s (%zu location available)", bot->GetGUIDLow(), bot->GetName(), nodeFrom->name[0], nodeTo->name[0], nodes.size());
    bot->SetMoney(money);
...
{  
    ...
    if (line == "clear")
    {
        ...
        /* SendMessageToPlayer not in vmangos */;
        return;
    }
    else if (line == "undo")
    {
    ...
    else
    {
        ...
        if (line.find("*") == 0)
            ...
            bot->Say(line.c_str(), LANG_UNIVERSAL);
...
bool RpgTradeUsefulAction::IsTradingItem(uint32 entry)
{
    ...
    for (uint8 i = 0; i < 6; i++)
    {
        Item* tradeItem = nullptr /* TradeData not fully available */;
...
bool RpgTradeUsefulAction::Execute(Event& event)
{
    ...
    if (ai->IsRealPlayer() && !bot->GetTradeData()) //Start the trade from the other side to open the window
    {
        ...
        player->GetSession()->HandleInitiateTradeOpcode(MakeTypedPacket<WorldPackets::Trade::InitiateTrade>(packet));
    }
    ...
    if (isTrading)
    {
        if (IsTradingItem(item->GetEntry())) //Did we manage to add the item to the trade?
        {
            if (bot->GetGroup() && bot->GetGroup()->IsMember(guidP))
                ...
                bot->Say(("You can use this " + chat->formatItem(item) + " better than me, " + player->GetName() + ".").c_str(), (bot->GetTeam() == ALLIANCE ? LANG_COMMON : LANG_ORCISH));

            if (!urand(0, 4) || items.size() < 2) //Complete the trade if we have no more items to trade.
            {
                //bot->Say(("End trade with" + chat->formatWorldobject(player)).c_str(), (bot->GetTeam() == ALLIANCE ? LANG_COMMON : LANG_ORCISH));
                WorldPacket p;
                uint32 status = TRADE_STATUS_TRADE_ACCEPT;
                p << status;
                bot->GetSession()->HandleAcceptTradeOpcode(MakeTypedPacket<WorldPackets::Trade::AcceptTrade>(p));
            }
        }
        //else
        //   bot->Say(("Start trade with" + chat->formatWorldobject(player)).c_str(), (bot->GetTeam() == ALLIANCE ? LANG_COMMON : LANG_ORCISH));
...
bool RpgEnchantAction::Execute(Event& event)
{
    ...
    std::shuffle(enchantSpells.begin(), enchantSpells.end(), std::mt19937{std::random_device{}()});
    ...
    for (auto& spellId : enchantSpells)
    {
        ...
        if (isTrading)
        {
            if (false /* TradeData not fully available */) //Did we manage to add the item to the trade?
            {
                uint32 duration;
                Unit* target = nullptr;
                ...
                if (didCast)
                {
                    ...
                    if (bot->GetGroup() && bot->GetGroup()->IsMember(guidP))
                        ...
                        bot->Say(("Let me enchant this " + chat->formatItem(item) + " with " + chat->formatSpell(spellId) + " for you " + player->GetName() + ".").c_str(), (bot->GetTeam() == ALLIANCE ? LANG_COMMON : LANG_ORCISH));
