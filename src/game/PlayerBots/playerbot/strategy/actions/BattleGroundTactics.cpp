...
static std::map<uint32, uint32> botSelectedObjectives;
...
ai::PositionMap& posMap = context->GetValue<ai::PositionMap&>("position")->Get();
    ...
    case BATTLEGROUND_AB:
        {
            ...
            std::set<uint32> uniqueObjectives;
            ...
            if (!defender)
            {
                std::set<uint32> enemyObjectives;
                std::set<uint32> neutralObjectives;
            ...
            if (!defender && uniqueObjectives.empty())
            {
                std::set<uint32> friendlyContestedObjectives;
                std::set<uint32> friendlyOccupiedObjectives;
                ...
                for (const auto& objective : AB_AttackObjectives)
                {
                    ...
                    if (isFriendlyContested)
                        friendlyContestedObjectives.insert(objective.first);
                    else if (isFriendlyOccupied)
                        friendlyOccupiedObjectives.insert(objective.first);
                }
            ...
            uint32 BgObjectiveNode = 0;
            bool hasObjective = false;

            // Keep the current objective if that AB node is still a valid candidate.
            auto selectedItr = botSelectedObjectives.find(botGUID);

            if (selectedItr != botSelectedObjectives.end() && uniqueObjectives.find(selectedItr->second) != uniqueObjectives.end())
            {
                BgObjectiveNode = selectedItr->second;
                hasObjective = true;
            }

            // Current objective is no longer valid, so choose a new one.
            if (!hasObjective && !uniqueObjectives.empty())
            {
                std::vector<uint32> objectives(uniqueObjectives.begin(), uniqueObjectives.end());

                BgObjectiveNode = objectives[urand(0, objectives.size() - 1)];

                botSelectedObjectives[botGUID] = BgObjectiveNode;
    ...
    case BATTLEGROUND_EY: //Role < 4: Defender, else Attacker. In the beginning split for all points. Afterwards pick random strategies
    {
            ...
            else if (role < 8) //Attack and Defend
            {
                while (BgObjective == nullptr)
                {
                    ...
                    { //Capture front objectives before attacking back objectives
                        //sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Bot #%d %s:%d <%s>: Get Front Objectives", bot->GetGUIDLow(), bot->GetTeam() == ALLIANCE ? "A" : "H", bot->GetLevel(), bot->GetName());
                        if (role < 6) {
                    ...
                    { //Now capture all objectives with priority on back
                        //sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Bot #%d %s:%d <%s>: Get All Objectives", bot->GetGUIDLow(), bot->GetTeam() == ALLIANCE ? "A" : "H", bot->GetLevel(), bot->GetName());
                        if (role < 4)
                        {
                            BgObjective = bot->GetMap()->GetGameObject(bg->GetSingleGameObjectGuid(attackObjectivesFront[0], 0));
                        }
        ...
        if (bot->HasAura(EY_SPELL_NETHERSTORM_FLAG))
        {
            ...
            if (!bg->IsActiveEvent(attackObjectivesFront[0], rootTeamIndex) && !bg->IsActiveEvent(attackObjectivesFront[1], rootTeamIndex) && !bg->IsActiveEvent(attackObjectivesBack[0], rootTeamIndex) && !bg->IsActiveEvent(attackObjectivesBack[1], rootTeamIndex))
            { //Retreat with flag
                //sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Bot #%d %s:%d <%s>: Retreat with flag", bot->GetGUIDLow(), bot->GetTeam() == ALLIANCE ? "A" : "H", bot->GetLevel(), bot->GetName());
                if (rootTeam == HORDE)
                {
                    areaTrigger = 0;
            ...
            { //Deliver flag
                //sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Bot #%d %s:%d <%s>: Deliver flag", bot->GetGUIDLow(), bot->GetTeam() == ALLIANCE ? "A" : "H", bot->GetLevel(), bot->GetName());
...
bool BGTactics::moveToObjective()
{
    ...
    if (bgType == BATTLEGROUND_AB)
        {
            ...
            if (selectedItr != botSelectedObjectives.end())
            {
                uint32 node = selectedItr->second;
                ...
                if (isFriendlyContested)
                {
                    // Our attack on this node has succeeded.
                    botSelectedObjectives.erase(botGUID);
                    botObjectiveSelectionTime.erase(botGUID);
    ...
    else
    {
        ...
        if (bot->GetDistance(pos.x, pos.y, pos.z) < 5.0f)
        {
            if (bgType == BATTLEGROUND_AB)
                resetObjective();
...
bool BGTactics::selectObjectiveWp(std::vector<BattleBotPath*> const& vPaths)
{
    ...
    for (const auto& pPath : vPaths)
    {
        ...
        for (uint32 i = 0; i < pPath->size(); ++i)
        {
            ...
            if (distanceFromMeToPoint < maxDistanceToPoint && distanceFromMeToPoint < closestDistanceFromMeToPoint)
            {
        ...
        if (nearestPoint != pPath->size() - 1)
        {
            BattleBotWaypoint& lastPoint = (*pPath)[pPath->size() - 1];

            float const distanceFromPathEndToTarget = WorldPosition(0, pos.x, pos.y, pos.z, 0).distance(WorldPosition(0, lastPoint.x, lastPoint.y, lastPoint.z, 0));

            if (distanceFromPathEndToTarget < closestDistanceToTarget)
            {
                reverse = false;
                pClosestPath = pPath;
        ...
        if (nearestPoint != 0)
        {
            ...
            float const distanceFromPathBeginToTarget = WorldPosition(0, pos.x, pos.y, pos.z, 0).distance(WorldPosition(0, firstPoint.x, firstPoint.y, firstPoint.z, 0));

            if (distanceFromPathBeginToTarget < closestDistanceToTarget)
            {
                reverse = true;
                pClosestPath = pPath;
                closestPoint = nearestPoint;
...
std::vector<uint32>::const_iterator f = find(vFlagIds.begin(), vFlagIds.end(), go->GetEntry());
        ...
        if (bgType == BATTLEGROUND_AB)
        {
            ...
            for (const auto& objective : AB_AttackObjectives)
            {
                ...
                if (isFriendlyOccupied || isFriendlyContested)
                {
                    ai::PositionMap& posMap = context->GetValue<ai::PositionMap&>("position")->Get();
                    ...
                    if (pos.isSet())
                    {
                        float dx = pos.x - go->GetPositionX();
                        float dy = pos.y - go->GetPositionY();
...
bool BGTactics::IsLockedInsideKeep()
{
    ...
    if (closestPortal)
    {
        ...
        {
            WorldPackets::Misc::GameObjectUse packet;
            packet.guid = closestPortal->GetObjectGuid();
            bot->GetSession()->HandleGameObjectUseOpcode(packet);
            return true;
        }
        else
