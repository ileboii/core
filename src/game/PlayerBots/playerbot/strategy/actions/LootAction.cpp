...
bool LootAction::Execute(Event& event)
{
    ...
    if (!prevLoot.IsEmpty() && prevLoot.guid != lootObject.guid)
    {
        ...
        bot->GetSession()->HandleLootReleaseOpcode(MakeTypedPacket<WorldPackets::Loot::LootRelease>(packet));
    }
    ...
    if (lootObject.skillId == SKILL_HERBALISM)
    {
        ...
        if (go && sObjectMgr.IsGameObjectForQuests(lootObject.guid.GetEntry()))
        {
            if (go->ActivateToQuest(bot))
            {
                ...
                isForQuest = !lootItems.empty() || go->GetGoState() != GO_READY;
            }
        }
...
bool StoreLootAction::Execute(Event& event)
{
    ...
    if (gold > 0)
    {
        ...
        bot->GetSession()->HandleLootMoneyOpcode(MakeNullPacket(packet));
    }
    ...
    for (uint8 i = 0; i < items; ++i)
    {
        ...
        packet << itemindex;
        bot->GetSession()->HandleAutostoreLootItemOpcode(MakeTypedPacket<WorldPackets::Loot::AutoStoreLootItem>(packet));
...
bool StoreLootAction::IsLootAllowed(ItemQualifier& itemQualifier, PlayerbotAI *ai)
{
    ...
    {
        uint32 entry = ai->GetBot()->GetUInt32Value(PLAYER_QUEST_LOG_1_1 + slot * MAX_QUEST_OFFSET + QUEST_ID_OFFSET);
