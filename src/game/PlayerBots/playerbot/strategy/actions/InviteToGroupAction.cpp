...
namespace ai
{
    bool InviteToGroupAction::Invite(Player* inviter, Player* player)
    {
        ...
        p << player->GetName();
        p << roles_mask;
        inviter->GetSession()->HandleGroupInviteOpcode(MakeTypedPacket<WorldPackets::Group::GroupInvite>(p));
    ...
    bool LfgAction::Execute(Event& event)
    {
        ...
        allowedClassNr = AllowedClassRoleNr(requester, stoi(param.empty() ? "5" : param));
            ...
            if (sPlayerbotAIConfig.inviteChat && (sRandomPlayerbotMgr.IsFreeBot(bot) || !ai->HasActivePlayerMaster()))
            {
                ...
                else
                {
                    ...
                    if (group && group->isRaidGroup())
                        bot->Say(BOT_TEXT2("join_raid", placeholders).c_str(), (bot->GetTeam() == ALLIANCE ? LANG_COMMON : LANG_ORCISH));
                    else
                        bot->Say(BOT_TEXT2("join_group", placeholders).c_str(), (bot->GetTeam() == ALLIANCE ? LANG_COMMON : LANG_ORCISH));
                }
            }
