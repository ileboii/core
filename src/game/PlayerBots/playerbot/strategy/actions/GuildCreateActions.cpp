...
bool BuyPetitionAction::Execute(Event& event)
{
    ...
    for (std::list<ObjectGuid>::iterator i = vendors.begin(); i != vendors.end(); ++i)
    {
        ...
        bot->GetSession()->HandlePetitionBuyOpcode(MakeTypedPacket<WorldPackets::Petition::PetitionBuy>(data));
...
bool PetitionOfferAction::Execute(Event& event)
{
    ...
    bot->GetSession()->HandleOfferPetitionOpcode(MakeTypedPacket<WorldPackets::Petition::OfferPetition>(data));
...
bool PetitionOfferNearbyAction::Execute(Event& event)
{
    ...
    for (auto& i : nearGuids)
    {
        ...
        if (sPlayerbotAIConfig.inviteChat && sServerFacade.GetDistance2d(bot, player) < sPlayerbotAIConfig.spellDistance && (sRandomPlayerbotMgr.IsFreeBot(bot) || !ai->HasActivePlayerMaster()))
        {
            ...
            if(urand(0,3))
                bot->Say(BOT_TEXT2("Hey %name do you want create a guild together?", placeholders).c_str(), (bot->GetTeam() == ALLIANCE ? LANG_COMMON : LANG_ORCISH));
            else
                bot->Say(BOT_TEXT2("Hey do you want to form a guild?", placeholders).c_str(), (bot->GetTeam() == ALLIANCE ? LANG_COMMON : LANG_ORCISH));
        }
...
bool PetitionTurnInAction::Execute(Event& event)
{
    ...
    for (std::list<ObjectGuid>::iterator i = vendors.begin(); i != vendors.end(); ++i)
    {
        ...
        bot->GetSession()->HandleTurnInPetitionOpcode(MakeTypedPacket<WorldPackets::Petition::TurnInPetition>(data));
