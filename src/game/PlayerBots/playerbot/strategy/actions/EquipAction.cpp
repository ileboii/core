...
void EquipAction::EquipItem(PlayerbotAI* ai, Player* requester, Item* item, bool silent)
{
    ...
    else
    {
        ...
        if (!equipedBag) 
        {
            ...
            bot->GetSession()->HandleAutoEquipItemOpcode(MakeTypedPacket<WorldPackets::Item::AutoEquipItem>(packet));
        }
    }
...
bool EquipUpgradesAction::Execute(Event& event)
{
    ...
    else if (event.getSource() == "item push result")
    {
        ...
        if (!data.empty())
        {
            ...
            if (itemProto && (itemProto->Class == ITEM_CLASS_WEAPON || 
                              itemProto->Class == ITEM_CLASS_ARMOR ||
                              itemProto->Class == ITEM_CLASS_CONTAINER))
            {
                valid = true;
            }
        }
    ...
    for (auto& item : items)
    {
        ...
        {
            sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Bot #%d <%s> auto equips item %d (%s)", bot->GetGUIDLow(), bot->GetName(), item->GetProto()->ItemId, usage == ItemUsage::ITEM_USAGE_EQUIP ? "better than current" : usage == ItemUsage::ITEM_USAGE_BAD_EQUIP ? "wrong item but empty slot" : "");
    ...
    if (didEquip && bot->CanDualWield())
    {
            ...
            && mh->GetProto()->InventoryType != INVTYPE_2HWEAPON)
        {
            ...
            if (ohMaxDmg > mhMaxDmg)
            {
                ...
                sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Bot #%d <%s> swapped MH/OH weapons to put higher top-end damage (%.1f) in main hand",
