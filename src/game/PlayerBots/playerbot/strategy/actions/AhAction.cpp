...
bool AhAction::PostItem(Player* requester, Item* item, uint32 price, Unit* auctioneer, uint32 time)
{
    ...
    bot->GetSession()->HandleAuctionSellItem(MakeTypedPacket<WorldPackets::AuctionHouse::AuctionSellItem>(packet));
...
bool AhBidAction::ExecuteCommand(Player* requester, std::string text, Unit* auctioneer)
{
    ...
    AuctionHouseObject::AuctionEntryMap* pAuctionMap = auctionHouse->GetAuctions(); AuctionHouseObject::AuctionEntryMap const& map = *pAuctionMap;
    ...
    if (text == "vendor")
    {
        ...
        for (uint32 i = 0; i < checkNumAuctions; i++)
        {
            ...
            case ItemUsage::ITEM_USAGE_AH:
            {
                ...
                bool isWorthBuyingFromAhToResellAtAH = ItemUsageValue::IsWorthBuyingFromAhToResellAtAH(sObjectMgr.GetItemPrototype(auction->itemTemplate), totalCost, 1 /* itemCount not in vmangos */);
                pmo.reset();
            ...
            case ItemUsage::ITEM_USAGE_VENDOR:
                ...
                if (totalCost / 1 /* itemCount not in vmangos */ >= (int32)sObjectMgr.GetItemPrototype(auction->itemTemplate)->SellPrice)
                    continue;
                power = 1000;
                break;
        ...
        for (auto auctionPower : auctionPowers)
        {
            ...
            std::string reason = ItemUsageValue::ReasonForNeed(usage, auction, 1 /* itemCount not in vmangos */, bot);            
    ...
    for (auto curAuction : map)
    {
        ...
        uint32 power = 1 /* itemCount not in vmangos */;
        power *= 1000;
        power /= cost;
...
bool AhBidAction::BidItem(Player* requester, AuctionEntry* auction, uint32 price, Unit* auctioneer, bool isBuyout, std::string reason)
{
    ...
    uint32 count = 1 /* itemCount not in vmangos */;
