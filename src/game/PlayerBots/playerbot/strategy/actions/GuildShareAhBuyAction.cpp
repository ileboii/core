...
uint32 GuildShareAhBuyAction::CountMailboxItems(uint32 itemId)
{
    ...
    for (PlayerMails::iterator itr = (PlayerMails::iterator()); itr != (PlayerMails::iterator()); ++itr)
    {
        ...
        {
            if (itemItr->itemId == itemId)
            {
                Item* mailItem = nullptr /* GetMItem not in vmangos */;
...
bool GuildShareAhBuyAction::Execute(Event& event)
{
    ...
    auto* bounds_map_ptr = auctionHouse->GetAuctions();
    for (auto itr = bounds_map_ptr->begin(); itr != bounds_map_ptr->end(); ++itr)
    {
