...
bool CastCustomSpellAction::Execute(Event& event)
{
    ...
    if (!pSpellInfo->EffectItemType[0] || pSpellInfo->Effect[0] != SPELL_EFFECT_CREATE_ITEM)
    {
    ...
    if (!bot->GetTrader() && !canCast)
    {
        ...
        args["%fail_reason"] = BOT_TEXT2("spell_failed", args);
...
bool CraftRandomItemAction::Execute(Event& event)
{
    ...
    std::shuffle(spellIds.begin(), spellIds.end(),std::mt19937{std::random_device{}()});
