...
namespace ai
{
    ...
    {
    public:
        BotUseItemSpell(Unit* caster, SpellEntry const* info, bool triggered, ObjectGuid originalCasterGUID = ObjectGuid(), SpellEntry const* triggeredBy = nullptr, bool itemCheats = false) : Spell(caster, info, triggered, originalCasterGUID, triggeredBy), itemCheats(itemCheats) {};

        static BotUseItemSpell* Create(Unit* caster, SpellEntry const* info, bool triggered, ObjectGuid originalCasterGUID = ObjectGuid(), SpellEntry const* triggeredBy = nullptr, bool itemCheats = false)
        {
            if (!caster || !info)
                return nullptr;

            if (info != sSpellMgr.GetSpellEntry(info->Id))
                return nullptr;
