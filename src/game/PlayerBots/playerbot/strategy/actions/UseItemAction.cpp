...
static const std::string LOS_GOS_PARAM = "los gos";
        ...
        switch (m_spellInfo->Effect[i])
        {
            ...
            case SPELL_EFFECT_OPEN_LOCK:
            {
                ...
                if (GameObject* go = m_targets.getGOTarget())
                {
                    ...
                    if (false /* IsInUse not in vmangos */)
                        return false;
                ...
                SpellCastResult res = SPELL_CAST_OK /* CanOpenLock stub */;
...
bool UseAction::Execute(Event& event)
{
    ...
    else
    {
        ...
        if (!items.empty())
        {
            ...
            if (items.size() > 1)
            {
                targetItem = nullptr /* GetItemByEntry not in vmangos */;
            }
        }
...
bool UseAction::UseGameObject(Player* requester, Event& event, GameObject* gameObject)
{
    ...
    if (!sServerFacade.isSpawned(gameObject) || false || gameObject->GetGoState() != GO_STATE_READY)
    {
...
bool UseAction::OpenItem(Player* requester, Item* item)
{
    ...
    if (!(item->GetProto()->Flags & ITEM_FLAG_LOOTABLE))
        ...
        *packet << item->GetSlot();
        bot->GetSession()->SendPacket(&*packet); // queue the packet to get around race condition
        return true;
}
...
bool UseItemIdAction::isUseful()
{
    ...
    {
        sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "UseItemIdAction::isUseful with qualifier %s, should be number.", getQualifier().c_str());
        return false;
    }
