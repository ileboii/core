...
namespace ai
{
    ...
    public:
        virtual bool Execute(Event& event) override
        {
            ...
            sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Bot #%d %s:%d <%s> released", bot->GetGUIDLow(), bot->GetTeam() == ALLIANCE ? "A" : "H", bot->GetLevel(), bot->GetName());

            WorldPacket packet(CMSG_REPOP_REQUEST);
            packet << uint8(0);
            bot->GetSession()->HandleRepopRequestOpcode(MakeNullPacket(packet));
    ...
    public:
        ...
        virtual bool Execute(Event& event) override
        {
            sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Bot #%d %s:%d <%s> auto released", bot->GetGUIDLow(), bot->GetTeam() == ALLIANCE ? "A" : "H", bot->GetLevel(), bot->GetName());
    ...
    public:
        virtual bool Execute(Event& event) override
        {
            ...
            sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Repop bot #%d %s:%d <%s>", bot->GetGUIDLow(), bot->GetTeam() == ALLIANCE ? "A" : "H", bot->GetLevel(), bot->GetName());
            ...
            if (!ai->HasRealPlayerMaster())
            {
                ...
                {
                    sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Repop: Removing bot #%d %s:%d <%s> from group", bot->GetGUIDLow(), bot->GetTeam() == ALLIANCE ? "A" : "H", bot->GetLevel(), bot->GetName());
    ...
    public:
        bool Execute(Event& event) override
        {
            ...
            bot->GetSession()->HandleSelfResOpcode(MakeNullPacket(packet));
            return true;
        }
