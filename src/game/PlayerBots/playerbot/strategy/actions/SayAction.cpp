...
void ChatReplyAction::GetAIChatPlaceholders(std::map<std::string, std::string>& placeholders, Unit* unit, const std::string preFix, Player* observer)
{
    ...
    if (unit->IsCreature())
    {
        ...
        std::string npcTextStr = placeholders["<" + preFix + " gossip>"];
        ...
        for (auto& gossip = pMenuBounds.first; gossip != pMenuBounds.second; gossip++)
        {
            ...
            /* NpcTextOption has no Text in vmangos */
        }
        ...
        if (textId)
        {
            ...
            if (gos) { /* NpcTextOption has no Text in vmangos */ }
        }

        for (auto& gossip = pMenuItemBounds.first; gossip != pMenuItemBounds.second; gossip++)
        {
            npcTextStr += " " + gossip->second.option_text;
        }
        ...
        PlayerbotTextMgr::ReplacePlaceholders(npcTextStr, replace);

        placeholders["<" + preFix + " gossip>"] = npcTextStr;
    }
}

WorldPacket ChatReplyAction::GetPacketTemplate(uint16 op, uint32 type, Unit* sender, Unit* target, std::string channelName)
{
    ...
    for (auto& line : lines)
    {
        ...
        if ((!isEmote || !emoteTemplate.empty()) && !sentence.empty())
        {
            ...
            if (timeDiff)
            {
                ...
                {
                    delay = 0;
                    sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "delay packet removed: %lu", delay);
                }
                else
                {
                    delay -= timeDiff;
                    sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "delay packet reduced to %lu", delay);
                }
                timeDiff = 0;
            }
