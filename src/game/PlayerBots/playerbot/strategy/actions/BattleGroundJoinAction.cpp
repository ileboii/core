...
bool BGJoinAction::Execute(Event& event)
{
    ...
    if (!queueType) // force join to fill bg
    {
        ...
        bracketId = Player::GetBattleGroundBracketIdFromLevel(bgTypeId, bot->GetLevel());
#endif
#ifdef MANGOSBOT_TWO
        ...
        //if (!BmEntry)
        //{
        //    sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "Bot #%d <%s> could not find Battlemaster for %d", bot->GetGUIDLow(), bot->GetName(), bgTypeId);
        //    return false;
        //}
        ...
        if (ArenaType type = sServerFacade.BgArenaType(queueTypeId))
        {
            ...
            {
                sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Team %dv%d, captain #%d <%s>: error making arena group!", uint32(type), uint32(type), bot->GetGUIDLow(), bot->GetName());
                return false;
            }
...
bool BGJoinAction::gatherArenaTeam(ArenaType type)
{
    ...
    if (!arenateam)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Bot #%d <%s>: No proper arena team found!", bot->GetGUIDLow(), bot->GetName());
        return false;
    }
    ...
    for (ArenaTeam::MemberList::iterator itr = arenateam->GetMembers().begin(); itr != arenateam->GetMembers().end(); ++itr)
    {
        ...
        if (offline)
        {
            ...
            {
                sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Team #%d <%s>: error logging in an offline member!", arenateam->GetId(), arenateam->GetName().c_str());
                return false;
            }
            else
                continue;
    ...
    {
        sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Team #%d <%s> has not enough members for match", arenateam->GetId(), arenateam->GetName().c_str());
        return false;
    }
    ...
    if (!leaderGroup)
    {
        ...
        {
            sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Team #%d <%s>: Can't create group for arena queue", arenateam->GetId(), arenateam->GetName().c_str());
            delete group;
            return false;
        }
        else
    ...
    sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Bot #%d <%s>: Leader of <%s>", bot->GetGUIDLow(), bot->GetName(), arenateam->GetName().c_str());
    ...
    for (auto i = begin(members); i != end(members); ++i)
    {
        ...
        sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Bot #%d <%s>: Member of <%s>", member->GetGUIDLow(), member->GetName(), arenateam->GetName().c_str());
    }

    if (leaderGroup && leaderGroup->GetMembersCount() >= needMembers)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Team #%d <%s>: Group is ready for match", arenateam->GetId(), arenateam->GetName().c_str());
        return true;
    }
    else if (leaderGroup && leaderGroup->GetMembersCount() < needMembers)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Team #%d <%s>: Group is not ready for match (not enough members)", arenateam->GetId(), arenateam->GetName().c_str());
        leaderGroup->Disband();
        return false;
    }
    return false;
...
bool BGJoinAction::canJoinBg(Player* player, BattleGroundQueueTypeId queueTypeId, BattleGroundBracketId bracketId)
{
    ...
    // check bracket
#ifndef MANGOSBOT_TWO
    if (Player::GetBattleGroundBracketIdFromLevel(bgTypeId, bot->GetLevel()) != bracketId)
        return false;
#endif
#ifdef MANGOSBOT_TWO
    BattleGround* bg = sBattleGroundMgr.GetBattleGroundTemplate(bgTypeId);
...
bool BGJoinAction::JoinQueue(uint32 type)
{
    ...
    {
        sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "Bot #%d <%s> : Battlemaster is not in map for BG %d", bot->GetGUIDLow(), bot->GetName(), bgTypeId);
        return false;
    }*/
...
bool BGLeaveAction::Execute(Event& event)
{
    ...
    if (bot->InBattleGround())
    {
        sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Bot #%d %s:%d <%s> leaves %s", bot->GetGUIDLow(), bot->GetTeam() == ALLIANCE ? "A" : "H", bot->GetLevel(), bot->GetName(), isArena ? "Arena" : "BG");

        WorldPacket leave(CMSG_LEAVE_BATTLEFIELD);
        leave << uint8(0) << uint8(0) << uint32(0) << uint16(0);
        bot->GetSession()->HandleLeaveBattlefieldOpcode(MakeTypedPacket<WorldPackets::Battleground::LeaveBattlefield>(leave));
    ...
    sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Bot #%d %s:%d <%s> leaves %s queue", bot->GetGUIDLow(), bot->GetTeam() == ALLIANCE ? "A" : "H", bot->GetLevel(), bot->GetName(), isArena ? "Arena" : "BG");
    ...
    {
        bot->GetSession()->HandleLeaveBattlefieldOpcode(MakeTypedPacket<WorldPackets::Battleground::LeaveBattlefield>(packet));
    }
    else
        bot->GetSession()->HandleBattleFieldPortOpcode(MakeTypedPacket<WorldPackets::Battleground::BattleFieldPort>(packet));
...
bool BGStatusAction::Execute(Event& event)
{
    ...
    if (!bg)
        return false;
    bracketId = Player::GetBattleGroundBracketIdFromLevel(_bgTypeId, bot->GetLevel());
#endif
#ifdef MANGOSBOT_TWO
    BattleGround* bg = sBattleGroundMgr.GetBattleGroundTemplate(_bgTypeId);
    if (!bg)
    ...
    {
        sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Bot #%u <%s> (%u %s): Received BG status TIME_REMOVE for %s %s", bot->GetGUIDLow(), bot->GetName(), bot->GetLevel(), bot->GetTeam() == ALLIANCE ? "A" : "H", isArena ? "Arena" : "BG", _bgType.c_str());
        ...
        sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Bot #%d %s:%d <%s> leaves %s - %s", bot->GetGUIDLow(), bot->GetTeam() == ALLIANCE ? "A" : "H", bot->GetLevel(), bot->GetName(), isArena ? "Arena" : "BG", _bgType.c_str());
    ...
    {
        sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Bot #%u %s:%d <%s>: Received BG status WAIT_QUEUE (wait time: %u) for %s %s", bot->GetGUIDLow(), bot->GetTeam() == ALLIANCE ? "A" : "H", bot->GetLevel(), bot->GetName(), Time2, isArena ? "Arena" : "BG", _bgType.c_str());
        // temp fix for crash
         //return true;
...
// TO DO: Fix for wotlk
...
#endif
                    sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Bot #%u %s:%d <%s>: Force join %s %s", bot->GetGUIDLow(), bot->GetTeam() == ALLIANCE ? "A" : "H", bot->GetLevel(), bot->GetName(), isArena ? "Arena" : "BG", _bgType.c_str());
                    
                    ai->Unmount();

                    action = 0x1;
                    // bg started so players should get invites by now
                    sRandomPlayerbotMgr.NeedBots[queueTypeId][bracketId][isArena ? isRated : (bot->GetTeam() == HORDE ? 1 : 0)] = false;
        ...
        if (leaveQ && ((bot->GetGroup() && bot->GetGroup()->IsLeader(bot->GetObjectGuid())) || !(bot->GetGroup() || ai->GetMaster())))
        {
            ...
            if (realPlayers)
                return false;
            sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Bot #%u %s:%d <%s> waited too long and leaves queue (%s %s).", bot->GetGUIDLow(), bot->GetTeam() == ALLIANCE ? "A" : "H", bot->GetLevel(), bot->GetName(), isArena ? "Arena" : "BG", _bgType.c_str());
            WorldPacket packet(CMSG_BATTLEFIELD_PORT, 20);
            action = 0;
#ifdef MANGOSBOT_ZERO
            packet << mapId << action;
...
//#endif
    ...
    {
        sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Bot #%u %s:%d <%s>: Received BG status IN_PROGRESS for %s %s", bot->GetGUIDLow(), bot->GetTeam() == ALLIANCE ? "A" : "H", bot->GetLevel(), bot->GetName(), isArena ? "Arena" : "BG", _bgType.c_str());
        return false;
    }
    if (statusid == STATUS_WAIT_JOIN) //bot may join
    {
        sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Bot #%u %s:%d <%s>: Received BG status WAIT_JOIN for %s %s", bot->GetGUIDLow(), bot->GetTeam() == ALLIANCE ? "A" : "H", bot->GetLevel(), bot->GetName(), isArena ? "Arena" : "BG", _bgType.c_str());
#ifndef MANGOSBOT_ZERO
        if (isArena)
        {
            ...
            if (!bgQueue.GetPlayerGroupInfoData(bot->GetObjectGuid(), &ginfo))
            {
                sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "Bot #%u %s:%d <%s>: Missing QueueInfo for %s %s", bot->GetGUIDLow(), bot->GetTeam() == ALLIANCE ? "A" : "H", bot->GetLevel(), bot->GetName(), isArena ? "Arena" : "BG", _bgType.c_str());
                return false;
            }
...
#ifdef MANGOSBOT_ZERO
        sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Bot #%d <%s> (%u %s) joined BG (%s)", bot->GetGUIDLow(), bot->GetName(), bot->GetLevel(), bot->GetTeam() == ALLIANCE ? "A" : "H", _bgType.c_str());
#else
        sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Bot #%d %s:%d <%s> joined %s - %s", bot->GetGUIDLow(), bot->GetTeam() == ALLIANCE ? "A" : "H", bot->GetLevel(), bot->GetName(), isArena ? "Arena" : "BG", _bgType.c_str());
#endif
...
bool BGStatusCheckAction::Execute(Event& event)
{
    ...
    bot->GetSession()->HandleBattlefieldStatusOpcode(MakeNullPacket(packet));
    sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Bot #%d <%s> (%u %s) : Checking BG invite status", bot->GetGUIDLow(), bot->GetName(), bot->GetLevel(), bot->GetTeam() == ALLIANCE ? "A" : "H");
    return true;
}
