...
namespace ai
{
    ...
    public:
        ...
        virtual bool Execute(Event& event) override
        {
            ...
            uint32 roles_mask = 0;
            p << roles_mask;
            bot->GetSession()->HandleGroupAcceptOpcode(NullClientPacket());
            ...
            if (master->GetPlayerbotAI()) //Copy formation from bot master.
            {
                if (sPlayerbotAIConfig.inviteChat && (sRandomPlayerbotMgr.IsFreeBot(bot) || !ai->HasActivePlayerMaster()))
                {
                    ...
                    if (guild && (master->GetGuildId() && master->GetGuildId() == bot->GetGuildId()))
                        ...
                        bot->Say(reply.c_str(), (bot->GetTeam() == ALLIANCE ? LANG_COMMON : LANG_ORCISH));
                }
