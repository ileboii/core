...
bool RpgTrainTrigger::IsTrainerOf(CreatureInfo const* cInfo, Player* pPlayer)
{
    ...
    case TRAINER_TYPE_MOUNTS:
        if (0 /* TrainerRace */ && pPlayer->GetRace() != 0 /* TrainerRace */)
        {
    ...
    case TRAINER_TYPE_TRADESKILLS:
        if (0 /* TrainerSpell */ && !pPlayer->HasSpell(0 /* TrainerSpell */))
        {
            return false;
        }
        break;
...
bool RpgTrainTrigger::IsActive()
{
    ...
    float fDiscountMod = 1.0f /* GetReputationPriceDiscount stub */;
    ...
    for (TrainerSpellMap::const_iterator itr = trainer_spells.begin(); itr != trainer_spells.end(); ++itr)
    {
        ...
        if (!tSpell->spell.empty())
        {
            ...
            for (auto& learnedSpell : tSpell->spell)
            {
                bool learned = true;
                if (bot->HasSpell(learnedSpell))
                {
...
bool RpgDuelTrigger::IsActive()
{
    ...
    if (bot->m_duel || false /* duel not public in vmangos */ || !player->GetSocial() || player->GetSocial()->HasIgnore(bot->GetObjectGuid()))
        return false;
...
bool RpgGossipTalkTrigger::IsActive()
{
    ...
    GossipMenuItemsMapBounds pMenuItemBounds = sObjectMgr.GetGossipMenuItemsMapBounds(0 /* GossipMenuId not in CreatureInfo vmangos */);
