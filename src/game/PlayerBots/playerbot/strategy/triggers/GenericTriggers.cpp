...
bool DeflectSpellTrigger::IsActive()
{
    ...
    if (spell)
    {
        ...
        if (tarSpellInfo)
        {
            attackSchool = SpellSchoolMask(tarSpellInfo->School);
...
bool TargetOfAttackerInRange::IsActive()
{
    ...
    return closestAttacker && bot->GetDistance(closestAttacker) <= (distance - sPlayerbotAIConfig.contactDistance);
}
