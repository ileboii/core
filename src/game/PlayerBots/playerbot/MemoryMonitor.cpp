...
void MemoryMonitor::Print()
{
    ...
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "MEMORY: %.2fgb", pmc.PrivateUsage/1000000000.0f);
    #endif

    for (auto& num : nums)
    {
        if(num.second > 1000000)
            sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "%s : %.2fm (%.2fk)", num.first.c_str(), num.second / 1000000.0f, (objectnumbersHist.back()[num.first] - objectnumbersHist.front()[num.first]) / 1000.0f);            
        else if(num.second > 1000)
            sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "%s : %.2fk (%.2fk)", num.first.c_str(), num.second / 1000.0f, (objectnumbersHist.back()[num.first] - objectnumbersHist.front()[num.first]) / 1000.0f);
        else
            sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "%s : %d (%d)", num.first.c_str(), num.second, (objectnumbersHist.back()[num.first] - objectnumbersHist.front()[num.first]));
    }
}
...
void MemoryMonitor::Browse()
{
    ...
    for (auto& t : here)
    {
        ...
        for (auto& p : top)
        {
            ...
            sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "<%d> \nfrom: %d\navg: %d\nto: %d\n%s", p.second, f, a, l, p.first.c_str());
        }
    }
}
