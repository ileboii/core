...
#endif
    default:
        sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "The race with ID %d does not have a naming category", race);
...
{
    sLog.Out(LOG_BASIC, LOG_LVL_DEBUG,  "Creating new random bot for class %d", cls);
    ...
    uint8 skinColor, faceId, hairStyle, hairColor, facialHair;
    Player::SelectRandomAppearance(race, gender, hairStyle, hairColor, faceId, facialHair, skinColor);

    WorldSession* session = new WorldSession(accountId, nullptr, SEC_PLAYER, 0, LOCALE_enUS);
    if (!session)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "BOTS: Unable to create session for random acc %d - name: \"%s\"; race: %u; class: %u", accountId, name.c_str(), race, cls);
        return false;
    }

    uint32 guidLow = sObjectMgr.GeneratePlayerLowGuid();
    if (!Player::SaveNewPlayer(session, guidLow, name, race, cls, gender,
            skinColor, faceId, hairStyle, hairColor, facialHair))
    {
        delete session;
    ...
    sLog.Out(LOG_BASIC, LOG_LVL_DEBUG,  "Random bot created for account %d - name: \"%s\"; race: %u; class: %u",
            accountId, name.c_str(), race, cls);
...
void RandomPlayerbotFactory::CreateRandomBots()
{
    ...
    if (sPlayerbotAIConfig.deleteRandomBotAccounts || delAccs)
    {
        ...
        if (!delFriends)
            sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "Deleting random bot characters without friends/guild...");
        else
            sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "Deleting all random bot characters...");
    ...
    for (uint32 accountNumber = 0; accountNumber < sPlayerbotAIConfig.randomBotAccountCount; ++accountNumber)
    {
        ...
        sLog.Out(LOG_BASIC, LOG_LVL_DEBUG, "Account %s created for random bots", accountName.c_str());
        bar.step();
    }
    ...
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "Loading available names...");
    ...
    {
        sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "The name database has not been updated. Run ai_playerbot_names.sql to update.");
    ...
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, ">> Loaded names for " SIZEFMTD " race/gender combinations.", freeNames.size());

    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "Creating random bot characters...");
    ...
    {
	sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "Unable to create all requested fixed class/race bots due to account character limits.");
	sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "The following class/race combination(s) were left uncreated:");
	...
	for(const auto& entry : remaining)
	{
	    ...
	    sLog.Out(LOG_BASIC, LOG_LVL_ERROR, " - Class %u, Race %u: %u bots remaining", cls, race, count);
	}
#ifdef MANGOSBOT_TWO
...
#endif
	sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "You need at least %u additional account(s) to fill the remaining fixed class/race combinations.", missingAccounts);
    }


    if (!botsCreated)
    {
	    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "No new random bots needed. Accounts: %zu, bots: %d.", sPlayerbotAIConfig.randomBotAccounts.size(), totalRandomBotChars);

        return;
    }

    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "%zu random bot accounts with %d characters available", sPlayerbotAIConfig.randomBotAccounts.size(), totalRandomBotChars+botsCreated);
}
...
void RandomPlayerbotFactory::CreateRandomGuilds()
{
    ...
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "Deleting random bot guilds...");
        uint32 counter = 0;
        ...
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "%d Random bot guilds deleted", counter);
    ...
    {
        sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "No leaders for random guilds available");
        return;
    }
    ...
    for (; guildNumber < maxNewGuilds; ++guildNumber)
    {
        ...
        {
            sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "Error creating random guild %s", guildName.c_str());
			continue;
        }
        ...
        sPlayerbotAIConfig.randomBotGuilds.push_back(guild->GetId());
        sLog.Out(LOG_BASIC, LOG_LVL_BASIC, "Random Guild <%s>, GM: %s", guildName.c_str(), player->GetName());
        newGuilds = true;
    }

    if (newGuilds)
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "Total Random Guilds: %d", (uint32)sPlayerbotAIConfig.randomBotGuilds.size());
}
...
std::string RandomPlayerbotFactory::CreateRandomGuildName()
{
    ...
    if (!result)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "No more names left for random guilds");
        return "";
    }
...
void RandomPlayerbotFactory::CreateRandomArenaTeams()
{
    ...
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "Deleting random bot arena teams...");
        for (std::vector<uint32>::iterator i = randomBots.begin(); i != randomBots.end(); ++i)
        {
        ...
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "Random bot arena teams deleted");
    ...
    for (; arenaTeamNumber < sPlayerbotAIConfig.randomBotArenaTeamCount; ++arenaTeamNumber)
    {
        ...
        {
            sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "No captains for random arena teams available");
            continue;
        }
        ...
        if (!player)
        {
            sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "Cannot find player for captain %d", player->GetGUIDLow());
            continue;
        }

        if (player->GetLevel() < PLAYER_MAX_LEVEL)
        {
            sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "Bot %d must be level %d to create an arena team", player->GetGUIDLow(), PLAYER_MAX_LEVEL);
            continue;
        }
        ...
        if (!results)
        {
            sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "No valid types for arena teams");
            return;
        }
        ...
        {
            sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "Error creating arena team %s", arenaTeamName.c_str());
            continue;
        }
        arenateam->SetCaptain(player->GetObjectGuid());
        sLog.Out(LOG_BASIC, LOG_LVL_BASIC, "Bot #%d %s:%d <%s>: captain of random Arena %s team - %s", player->GetGUIDLow(), player->GetTeam() == ALLIANCE ? "A" : "H", player->GetLevel(), player->GetName(), arenaTypeName.c_str(), arenateam->GetName().c_str());
        // set random emblem
        ...
        for (uint32 i = 0; i < 10; i++)
        {
            ...
            sLog.Out(LOG_BASIC, LOG_LVL_BASIC, "Bot #%d %s:%d <%s>: added to random Arena %s team - %s", member->GetGUIDLow(), member->GetTeam() == ALLIANCE ? "A" : "H", member->GetLevel(), member->GetName(), arenaTypeName.c_str(), arenateam->GetName().c_str());
        ...
        if (arenateam->GetMembersSize() < type)
        {
            sLog.Out(LOG_BASIC, LOG_LVL_BASIC, "Random Arena team %s %s: failed to get enough members, deleting...", arenaTypeName.c_str(), arenateam->GetName().c_str());
        ...
        sLog.Out(LOG_BASIC, LOG_LVL_BASIC, "Random Arena team %s %s: created", arenaTypeName.c_str(), arenateam->GetName().c_str());
    }

    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "%d random bot arena teams available", arenaTeamNumber);
}
...
std::string RandomPlayerbotFactory::CreateRandomArenaTeamName()
{
    ...
    if (!result)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "No more names left for random arena teams");
        return "";
    }
