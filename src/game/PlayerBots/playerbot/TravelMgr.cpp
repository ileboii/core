...
void TravelMgr::LoadAreaLevels()
{
...

    {
        ...
        if (result)
        {
            ...
            sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, ">> Loaded " SIZEFMTD " area levels.", areaLevels.size());
        }
        ...
        if(areaLevels.size() > loadedAreas.size())
            sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, ">> Generated " SIZEFMTD " areas.", areaLevels.size()- loadedAreas.size());
    }
}

void TravelMgr::SetMobAvoidArea()
{
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "-Apply mob avoidance maps");
    ...
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, ">> Modified navmap areas for %d maps.", sMapStorage.GetMaxEntry());
}

void TravelMgr::SetMobAvoidAreaMap(uint32 mapId) 
{
    // PathFinder not compatible - vmangos PathInfo needs Unit*
    FactionTemplateEntry const* humanFaction = sFactionTemplateStore.LookupEntry(1);
    FactionTemplateEntry const* orcFaction = sFactionTemplateStore.LookupEntry(2);
...
void TravelMgr::LoadQuestTravelTable()
{
    ...
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "Loading trainable spells.");
    GAI_VALUE(trainableSpellMap*, "trainable spell map");
    GAI_VALUE(std::vector<MountValue>, "full mount list");
    
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "Loading object locations.");

    EntryGuidps guidpMap = GAI_VALUE(EntryGuidps, "entry guidps");

    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "Finding possible travel destinations.");

    EntryQuestRelationMap eMap = GAI_VALUE(EntryQuestRelationMap, "entry quest relation");

    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "Creating travel destinations.");
    ...
    for (auto& [entry, relation] : eMap)
    {
        ...
        for (auto& [questId, flag] : relation)
        {
            ...
            {
                sLog.Out(LOG_BASIC, LOG_LVL_DEBUG, "Entry %d for quest %d has no valid location.", entry, questId);
                continue;
            }
    ...
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "Loading all travel locations.");
    ...
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "Loading Explore locations.");
    ...
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "Running analysis.");
        LogAnalysis::RunAnalysis();
    }

    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "Clearing log files.");
    ...
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "Loading travel nodes.");
    ...
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "Create creature overlay exports.");
...
# define my_sizeof(type) ((char *)(&type+1)-(char*)(&type))
    ...
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "Create go overlay exports.");
        for (auto& gameObjectPair : WorldPosition().getGameObjectsNear())
        {
            GameObjectData const gData = gameObjectPair->second;
            auto data = sObjectMgr.GetGameObjectTemplate(gData.id);
    ...
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "Create zone overlay exports.");
    ...
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "Create telecache overlay exports.");
    ...
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "Create travel destinations export.");
...
{
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "Finding populated grids.");
...
{
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "Loading Fish locations.");
    ...
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, ">> Loaded %u fish locations", count);
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, " ");
}

void TravelMgr::GetFishLocations()
{
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "Generating Fish locations.");
            ...
            for (uint8 cellx = 0; cellx < subCellPerGrid; cellx++)
            {
                for (uint8 celly = 0; celly < subCellPerGrid; celly++)
                {
                    ...
                    for (float angle = 0.0f; angle < 360.0f; angle += 45.0f)
                    {
                        ...
                        // uint32 area = 0 /* getArea not in vmangos */, fishSpot.getY(), fishSpot.getZ());

                        if (false /* nav area check disabled */)
                        {
                        //    sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "no area %d %f %f %f", mapId, fishSpot.getX(), fishSpot.getY(), fishSpot.getZ());
                            continue;
                        }
...
{
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "Saving Fish locations.");
