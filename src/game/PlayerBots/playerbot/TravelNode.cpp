...
float TravelNodePath::getCost(Unit* unit, uint32 cGold)
{
    ...
    if (bot)
    {
        ...
        if (getPathType() == TravelNodePathType::areaTrigger && pathObject)
        {
            ...
            AreaTriggerEntry const* at = sObjectMgr.GetAreaTrigger(pathObject);
            if (atEntry && at && atEntry->map_id == bot->GetMapId())
            {
                ...
                if (map)
                    if (at && false /* conditionId not in vmangos */ && !true /* condition check not available */)
                        return -1;
            }
        }
        ...
        if (getPathType() == TravelNodePathType::staticPortal && pathObject)
        {
            ...
            auto data = sObjectMgr.GetGameObjectTemplate(goEntry);
        ...
        if (getPathType() == TravelNodePathType::flightPath && pathObject)
        {
            ...
            if (taxiPath)
            {
                ...
                if (!false && taxiPath->price > cGold)
                    return -1;

                if (!false && !bot->GetTaxi().IsTaximaskNodeKnown(taxiPath->to))
                    return -1;
...
{
    for (uint32 i = 0; i < 0 /* sAreaTriggerStore not in vmangos - disabled */; i++)
    {
        if (areaTriggerId && areaTriggerId != i)
            continue;

        AreaTriggerEntry const* atEntry = sObjectMgr.GetAreaTrigger(i);
        if (!atEntry)
            continue;

        AreaTriggerEntry const* at = sObjectMgr.GetAreaTrigger(i);
        if (!at)
            continue;

        WorldPosition outPos = WorldPosition(at->map_id, at->x, at->y, at->z, 0.0f /* no target_orientation in vmangos */);
...
WorldPosition TravelPath::getNextPoint(WorldPosition startPos, float maxDist, TravelNodePathType& pathType, uint32& entry, bool onTransport, WorldPosition& telePosition)
{
    ...
    if (sPlayerbotAIConfig.transportTeleportType < 2 && startP->type == PathNodeType::NODE_TRANSPORT)
    {
        ...
        if (!onTransport)
        {
            telePosition = prevP->point; //We want stand here to walk on boat?
            cutTo(*startP, false);
            return nextPoint; //Bot should dock here.
        }

        for (auto p = startP; p != ed; p++) //Move along the transport path to the end of the boat ride.
        {
...
TravelNodeRoute TravelNodeMap::getRoute(WorldPosition startPos, WorldPosition endPos, std::vector<WorldPosition>& startPath, std::vector<WorldPosition>& endPath, Unit* unit)
{
    ...
    Player* debugBot = dynamic_cast<Player*>(unit);
    if (debugBot && debugBot->GetPlayerbotAI() && debugBot->GetPlayerbotAI()->HasStrategy("debug move", BotState::BOT_STATE_NON_COMBAT))
    {
        if (!startNodes.empty())
        {
            // Show the top 3 closest start nodes and their route counts
            std::vector<TravelNode*> sorted = startNodes;
            std::partial_sort(sorted.begin(), sorted.begin() + std::min(3u, (uint32)sorted.size()), sorted.end(),
                [startPos](TravelNode* i, TravelNode* j) { return i->getPosition()->sqDistance(startPos) < j->getPosition()->sqDistance(startPos); });

            for (uint32 i = 0; i < std::min(3u, (uint32)sorted.size()); i++)
            {
                sLog.Out(LOG_BASIC, LOG_LVL_BASIC, "[Bot %s]   startNode[%u]: '%s' at (%.1f,%.1f), routes=%u",
                    debugBot->GetName(), i, sorted[i]->getName().c_str(),
                    sorted[i]->getPosition()->getX(), sorted[i]->getPosition()->getY(),
                    sorted[i]->getRouteSize());
            }
        }
        if (!endNodes.empty())
        {
            std::vector<TravelNode*> sorted = endNodes;
            std::partial_sort(sorted.begin(), sorted.begin() + std::min(3u, (uint32)sorted.size()), sorted.end(),
                [endPos](TravelNode* i, TravelNode* j) { return i->getPosition()->sqDistance(endPos) < j->getPosition()->sqDistance(endPos); });

            for (uint32 i = 0; i < std::min(3u, (uint32)sorted.size()); i++)
            {
                sLog.Out(LOG_BASIC, LOG_LVL_BASIC, "[Bot %s]   endNode[%u]: '%s' at (%.1f,%.1f), routes=%u",
                    debugBot->GetName(), i, sorted[i]->getName().c_str(),
                    sorted[i]->getPosition()->getX(), sorted[i]->getPosition()->getY(),
                    sorted[i]->getRouteSize());
            }
...
{
    sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "Trying to load all maps and tiles for node generation. Please ignore any maps that could not be loaded.");
    for (uint32 i = 0; i < sMapStorage.GetMaxEntry(); ++i)
    {
        ...
        if (mapId == 0 || mapId == 1 || mapId == 530 || mapId == 571)
        {
            ...
            // loadAllMapTiles not in vmangos
#else
        ...
        {
            // loadMapInstance not in vmangos
        }
    }
    ...
    for (uint32 i = 0; i < sMapStorage.GetMaxEntry(); ++i)
    {
        ...
        for (const auto& entry : std::filesystem::directory_iterator(sWorld.GetDataPath() + "mmaps"))
        {
            if (entry.path().extension() == ".mmtile")
            {
                ...
                if (true /* IsMMapIsLoaded not in vmangos */)
...
void TravelNodeMap::generatePortalNodes()
{
    ...
    for (uint32 i = 0; i < sSpellMgr.GetMaxSpellId(); ++i)
    {
        ...
        {
            sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "%s adjusting height down from %f", pSpellInfo->SpellName[0], outPos.currentHeight());
...
{
    for (uint32 entry = 1; entry <= 32000; ++entry)
    {        
        auto data = sObjectMgr.GetGameObjectTemplate(entry);
        ...
        if (data && (data->type == GAMEOBJECT_TYPE_TRANSPORT || data->type == GAMEOBJECT_TYPE_MO_TRANSPORT))
        {
            ...
            else //Boats/Zepelins
            {
                ...
                {
                    TaxiPathNodeEntry const& p = (TaxiPathNodeEntry const&)path[pi];
                ...
                if (prevNode)
                {
                    ...
                    for (size_t pi2 = 0; pi2 < path.size(); ++pi2)
                    {
                        TaxiPathNodeEntry const& p = (TaxiPathNodeEntry const&)path[pi2];
                        WorldPosition pos = WorldPosition(p.mapid, p.x, p.y, p.z, 0);
...
{
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "-Generating Start nodes");
    generateStartNodes();
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "-Generating npc nodes");
    generateNpcNodes();
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "-Generating area trigger nodes");
    generateAreaTriggerNodes();
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "-Generating transport nodes");
    generateTransportNodes();
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "-Generating zone mean nodes");
    generateZoneMeanNodes();
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "-Generating static portal nodes");
    generatePortalNodes();
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "-Adding manually defined nodes");
    addManualNodes();
}
...
void TravelNodeMap::generateWalkPaths()
{
    ...
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, ">> Generated paths for " SIZEFMTD " nodes.", sTravelNodeMap.getNodes().size());
}
...
void TravelNodeMap::generateHelperNodes()
{
    ...
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "-Calculating walkable paths for %d new nodes.", uint32(sTravelNodeMap.getNodes().size() - old));
        generateWalkPaths();
    }
    ...
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "-Finding new nodes to reach %d nodes that can currently not be properly reached.", places_to_reach);
    ...
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, ">> Generated " SIZEFMTD " helpdernodes.", sTravelNodeMap.getNodes().size()-old);
}
...
void TravelNodeMap::generateTaxiPaths()
{
    for (uint32 i = 0; i < sTaxiPathStore.GetNumRows(); ++i)
    {
        ...
        if (nodes.size() > 0 && startNode->fDist(WorldPosition(((TaxiPathNodeEntry const&)nodes[0]).mapid, ((TaxiPathNodeEntry const&)nodes[0]).x, ((TaxiPathNodeEntry const&)nodes[0]).y, ((TaxiPathNodeEntry const&)nodes[0]).z, 0.0f)) > 0.1f)
            ppath.push_back(*startNode->getPosition());

        for (size_t ni = 0; ni < nodes.size(); ++ni)
        {
            TaxiPathNodeEntry const& n = (TaxiPathNodeEntry const&)nodes[ni];
...
void TravelNodeMap::removeLowNodes()
{
    ...
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "-Removed %d nodes had below 5 connections to other nodes.", (uint32)remNodes.size());
}
...
void TravelNodeMap::removeUselessPathMap(uint32 mapId)
{
    ...
    while (true)
    {
        ...
        sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "MapId %d Iteration %d, removed %d", mapId, it, rem);
    }
}
...
{   
    ...
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, ">> Calculated cost for " SIZEFMTD " paths.", calculations.size());
...
void TravelNodeMap::generatePaths(bool helpers)
{
    ...
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "-Calculating walkable paths");
    generateWalkPaths();

    if (helpers)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "-Generating helper nodes");
        generateHelperNodes();
    }

    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "-Removing useless nodes");
    removeLowNodes();
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "-Removing useless paths");
    removeUselessPaths();

    printMap();

    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "-Calculating path costs");
    calculatePathCosts();
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "-Generating taxi paths");
    generateTaxiPaths();
}
...
void TravelNodeMap::generateAll()
{
    ...
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "-Calculating mapoffset");
    calcMapOffset();

    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "-Generating maptransfers");
    ...
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "-Calculating coverage"); //This prevents crashes when bots from multiple maps try to calculate this on the fly.
...
void TravelNodeMap::saveNodeStore(bool force)
{
    ...
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, ">> Saved " SIZEFMTD " travelNodes.", anodes.size());

    {
        ...
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, ">> Saved %d travelNode Paths, %d points.", paths,points);
    }
}
...
{    
...

    {
        ...
        if (result)
        {
            ...
            sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, ">> Loaded " SIZEFMTD " travelNodes.", saveNodes.size());
        }
        else
        {
            hasToFullGen = true;
            sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, " ");
            sLog.Out(LOG_BASIC, LOG_LVL_ERROR, ">> Error loading travelNodes.");
            return;
        }
    }

    {
        ...
        if (result)
        {
            ...
            sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, ">> Loaded " SIZEFMTD " travelNode paths.", result->GetRowCount());
        }
        else
        {
            sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, " ");
            sLog.Out(LOG_BASIC, LOG_LVL_ERROR, ">> Error loading travelNode links.");
        }
    }

    {
        ...
        if (result)
        {
            ...
            sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, ">> Loaded " SIZEFMTD " travelNode paths points.", result->GetRowCount());
        }
        else
        {
            sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, " ");
            sLog.Out(LOG_BASIC, LOG_LVL_ERROR, ">> Error loading travelNode paths.");
        }
...
TravelNodeMap::PathFindResult TravelNodeMap::testPathToLoop(const WorldPosition& startPos, const WorldPosition& endPos, const Unit* bot, uint64 uid, std::vector<WorldPosition> route, std::string type) const
{
    ...
    // setAreaCost not available in VMaNGOS PathInfo
    ...
    else if (distRemaining < 5.0f)     // tune this ÃƒÆ’Ã…Â½ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¶ close in absolute terms
        status = "near miss 5y";
    else if (distRemaining < 10.0f) // tune this ÃƒÆ’Ã…Â½ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¶ close in absolute terms
