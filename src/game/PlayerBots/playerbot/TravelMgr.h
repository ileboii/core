...
// Pair hash for std::unordered_map with std::pair keys (replaces boost::hash)
struct PairHash {
...
namespace ai
{
	...
	{
	public:
		EntryTravelDestination(TravelDestinationPurpose purpose, int32 entry) : TravelDestination(), purpose(purpose), entry(entry) { if (entry > 0) creatureInfo = sObjectMgr.GetCreatureTemplate(entry); else goInfo = sObjectMgr.GetGameObjectTemplate(-1 * entry); }
	...
	private:
		...
		std::unordered_map<std::pair<uint32, uint32>, std::vector<MapTransfer>, PairHash> mapTransfersMap;
	};
}
