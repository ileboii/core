...
static std::vector<std::string> GetConfigValues(Config const& cfg, const std::string& name)
{
    ...
    std::string filename = cfg.GetFilename();
    ...
    while (std::getline(file, line))
    {
        ...
        if (keyLower.find(nameLower) != std::string::npos)
...
{
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "Initializing AI Playerbot by ike3, based on the original Playerbot by blueboy");

    if (!config.LoadFromFile(_D_AIPLAYERBOT_CONFIG))
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "AI Playerbot is Disabled. Unable to open configuration file aiplayerbot.conf");
        return false;
    }
    ...
    if (!enabled)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "AI Playerbot is Disabled in aiplayerbot.conf");
        return false;
    }
    ...
    std::vector<std::string> criteriaValues = GetConfigValues(config, "AiPlayerbot.LoginCriteria");
    ...
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "Loading Race/Class probabilities");
    ...
    if (useFixedClassRaceCounts)
    {
        ...
        for (uint32 race = 1; race < MAX_RACES; ++race)
        {
            ...
            if (val >= 0)
                sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "Fixed class/race counts does not yet support '%s' (race-only). This config entry will be ignored.", raceKey.c_str());
        }
        ...
        for (uint32 cls = 1; cls < MAX_CLASSES; ++cls)
        {
            ...
            if (val >= 0)
                sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "Fixed class/race counts does not yet support '%s' (class-only). This config entry will be ignored.", classKey.c_str());
        }
    ...
    std::vector<std::string> values = GetConfigValues(config, "AiPlayerbot.WorldBuff");

    if (values.size())
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "Loading WorldBuffs");
    ...
    playerbotsKillXPrate = config.GetFloatDefault("AiPlayerbot.KillXPRate", 1.0f);
    ...
    try {
        ...
        sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "Unable to parse LLMApiEndpoint url: %s", e.what());
    }
    ...
    {
        sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "# AiPlayerbot.LLMResponseStartPattern = %s", llmResponseStartPattern.c_str());
        sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "# AiPlayerbot.LLMResponseEndPattern = %s", llmResponseEndPattern.c_str());
        sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "# AiPlayerbot.LLMResponseDeletePattern = %s", llmResponseDeletePattern.c_str());
        sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "# AiPlayerbot.LLMResponseSplitPattern = %s", llmResponseSplitPattern.c_str());
    }
    ...
    try {
        ...
        sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "Regex error in %s: %s", llmResponseStartPattern.c_str(), e.what());
    }
    ...
    try {
        ...
        sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "Regex error in %s: %s", llmResponseEndPattern.c_str(), e.what());
    }
    ...
    try {
        ...
        sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "Regex error in %s: %s", llmResponseDeletePattern.c_str(), e.what());
    }
    ...
    try {
        ...
        sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "Regex error in %s: %s", llmResponseSplitPattern.c_str(), e.what());
    }
    ...
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "Loading free bots.");
    ...
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "Loading area levels.");
    sTravelMgr.LoadAreaLevels();
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "Loading spellIds.");
    ...
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "Loading Quest Detail Data...");
        sTravelMgr.LoadQuestTravelTable();
    }

    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "Loading named locations...");
    ...
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "        AI Playerbot initialized       ");
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "---------------------------------------");
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, " ");
    ...
    if (results)
    {
        do
        {
            ...
            do
            {
                ...
                {
                    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "Enabling always online for %s", charName.c_str());
...
void PlayerbotAIConfig::LoadLLMDefaultPrompts(const std::string& fileName)
{
    ...
    if (!file.is_open())
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "LLM default prompts file '%s' not found or unreadable.", fileName.c_str());
        return;
    }
    ...
    while (std::getline(file, line))
    {
        { auto p1 = line.find_first_not_of(" \t\n\r"); auto p2 = line.find_last_not_of(" \t\n\r"); line = (p1 == std::string::npos) ? "" : line.substr(p1, p2 - p1 + 1); };
        ...
        {
            sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "LLM prompts file '%s' contains invalid line (missing '::'): %s", fileName.c_str(), line.c_str());
            continue;
        }

        std::string name = line.substr(0, delim);
        std::string text = line.substr(delim + 2);
        { auto p1 = name.find_first_not_of(" \t\n\r"); auto p2 = name.find_last_not_of(" \t\n\r"); name = (p1 == std::string::npos) ? "" : name.substr(p1, p2 - p1 + 1); };
        { auto p1 = text.find_first_not_of(" \t\n\r"); auto p2 = text.find_last_not_of(" \t\n\r"); text = (p1 == std::string::npos) ? "" : text.substr(p1, p2 - p1 + 1); };

        if (name.empty())
        {
            sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "LLM prompts file '%s' contains empty name: %s", fileName.c_str(), line.c_str());
            continue;
        }
        ...
        if (!result)
        {
            sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "Character '%s' not found in characters DB while loading '%s'.", name.c_str(), fileName.c_str());
            continue;
        }
