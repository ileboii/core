...
BotPool PlayerBotLoginMgr::LoadBotsFromDb()
{
    ...
    sLog.Out(LOG_BASIC, LOG_LVL_DEBUG, "PlayerbotLoginMgr: %d accounts found.", uint32(accounts.size()));
    ...
    sLog.Out(LOG_BASIC, LOG_LVL_DEBUG, "PlayerbotLoginMgr: %d bots found.", uint32(botPool.size()));
...
void PlayerBotLoginMgr::FillLoginSpace(BotPool* pool, LoginSpace& space, FillStep step)
{
    ...
    for (uint32 level = 1; level < PLAYER_MAX_LEVEL + 1; ++level)
    {
...
BotInfos PlayerBotLoginMgr::FillLoginLogoutQueue(BotPool* pool, const RealPlayers& realPlayers)
{
    ...
    for (uint8 attempt = 0; attempt <= GetLoginCriteriaSize(); attempt++)
    {
        ...
        if (sPlayerBotLoginMgr.debug)
        {
            ...
            sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "PlayerbotLoginMgr: Attempt %d (%s), space left %d", attempt, variableCriteria.c_str(), loginSpace.totalSpace);
        }
