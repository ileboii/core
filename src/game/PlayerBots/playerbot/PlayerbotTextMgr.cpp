...
void PlayerbotTextMgr::LoadBotTexts()
{
    ...
    if (results)
    {
        do
        {
            ...
            for (uint8 i = 1; i < MAX_LOCALE; ++i)
            {
                text_locale[i] = fields[i + 3].GetString();
            }
            botTexts[name].push_back(BotTextEntry(name, text, text_locale, sayType, replyType));
            count++;
        } while (results->NextRow());
    }
    sLog.Out(LOG_BASIC, LOG_LVL_BASIC, "%d playerbot texts loaded", count);
}
...
std::string PlayerbotTextMgr::GetBotText(std::string name)
{
    if (botTexts.empty())
    {
        sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "Can't get bot text %s! No bots texts loaded!", name.c_str());
        return "";
    }
    if (botTexts[name].empty())
    {
        sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Can't get bot text %s! No bots texts for this name!", name.c_str());
        return "";
    }
...
std::string PlayerbotTextMgr::GetBotText(ChatReplyType replyType, std::map<std::string, std::string> placeholders)
{
    if (botTexts.empty())
    {
        sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "Can't get bot text reply %u! No bots texts loaded!", replyType);
        return "";
    }
    if (botTexts["reply"].empty())
    {
        sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Can't get bot text reply %u! No bots texts replies!", replyType);
        return "";
    }
