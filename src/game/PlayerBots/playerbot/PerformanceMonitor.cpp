...
void PerformanceMonitor::PrintStats(bool perTick, bool fullStack, bool showMap)
{
    ...
    if (!perTick)
    {
        ...
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "--------------------------------------[TOTAL BOT]------------------------------------------------------");
    }
    else
    {
        ...
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "---------------------------------------[PER TICK]------------------------------------------------------");
    }
    ...
    for (auto& [metric, namedData] : data)
    {
        ...
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "percentage   time    |   min  ..    max (     avg  of     count ) - type : name                        ");
        ...
        for (auto& stack : stacks)
        {
            ...
            {
                if (perTick)
                    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "%7.3f%% %9ums | %6u .. %6u (%9.2f of %10.2f) - %s    : %s", perc, (uint32)secs, pd.minTime, pd.maxTime, avg, amount, key.c_str(), disName.c_str());
                else
                    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "%7.3f%% %10.3fs | %6u .. %6u (%9.4f of %10u) - %s    : %s", perc, secs, pd.minTime, pd.maxTime, avg, (uint32)amount, key.c_str(), disName.c_str());
        ...
        {
            if (perTick)
                sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "%7.3f%% %9ums | %6u .. %6u (%9.4f of %10.2f) - %s    : %s", tPerc, (uint32)secs, tMin, tMax, avg, tCount, key.c_str(), "TOTAL");
            else
                sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "%7.3f%% %10.3fs | %6u .. %6u (%9.2f of %10u) - %s    : %s", tPerc, secs, tMin, tMax, avg, (uint32)tCount, key.c_str(), "TOTAL");
        }
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, " ");
    }
...
bool ChatHandler::HandlePerfMonCommand(char* args)
{
    if (!strcmp(args, "reset"))
    {
        ...
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "Performance monitor reset");
        return true;
    }
    ...
    if (!strcmp(args, "toggle"))
    {
        ...
        if (sPlayerbotAIConfig.perfMonEnabled)
            sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "Performance monitor enabled");
        else
            sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "Performance monitor disabled");
