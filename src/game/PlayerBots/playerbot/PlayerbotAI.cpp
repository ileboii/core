...
{
    this->bot = bot;
    // taxi cheater not supported in vmangos
...
void PlayerbotAI::UpdateAI(uint32 elapsed, bool minimal)
{
    ...
    {
        if (sServerFacade.IsInCombat(bot) || (master && IsSafe(master) && sServerFacade.IsInCombat(master) && sServerFacade.GetDistance2d(bot, master) < 30.0f))
        {
            NullClientPacket p;
    ...
    if (botMoving && !bot->IsBeingTeleported() && bot->IsInWorld())
    {
        ...
        if (lootGuid)
        {
            GuidPosition lootObject(lootGuid, bot);
    ...
    if (jumpTime && (jumpTime < curTime || (jumpTime + 10000 < curTime)))
    {
        ...
        if (!fallAfterJump)
        {
            ...
            QueuePacket(land);
            sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "%s: Jump: Landed, landTime: %u", bot->GetName(), curTime);
        ...
        else
        {
            ...
            if (useMoveFall && false /* MoveFall not in vmangos */)
            {
                jumpTime = 0;
                fallAfterJump = false;
                ResetJumpDestination();
                sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "%s: Jump: MoveFall activated", bot->GetName());
            }
            // simulate falling
            else
            {
                ...
                sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "%s: Jump: Falling simulated, height: %f, timeToLand %u", bot->GetName(), landingHeight, jumpTime);
            }
        }
    }
...
void PlayerbotAI::UpdateFaceTarget(uint32 elapsed, bool minimal)
{
    ...
    if (faceTargetUpdateDelay <= 0U)
    {
        ...
        if (IsStateActive(BotState::BOT_STATE_COMBAT))
        {
            ...
            if (!sServerFacade.isMoving(bot) && !bot->HasUnitState(UNIT_STATE_MOVING))
            {
...
bool PlayerbotAI::CanEnterArea(const AreaTriggerEntry* area)
{
    if (sRandomPlayerbotMgr.IsRandomBot(GetBot()))
    {
        DungeonPersistentState* state = bot->GetBoundInstanceSaveForSelfOrGroup(area->map_id);
        Map* map = sMapMgr.FindMap(area->map_id, state ? state->GetInstanceId() : 0);
        const MapEntry* mapEntry = sMapStorage.LookupEntry<MapEntry>(area->map_id);
...
#endif
        {
            if (false) // CanEnterNewInstance not in vmangos
            {
                return false;
            }
        }
        ...
        if (map && map->IsDungeon())
        {
            ...
            if (bot->GetMapId() != area->map_id)
            {
                ...
                // Bind Checks
#ifdef MANGOSBOT_ZERO
                InstancePlayerBind* pBind = bot->GetBoundInstance(area->map_id);
#elif MANGOSBOT_ONE
                InstancePlayerBind* pBind = bot->GetBoundInstance(area->map_id, bot->GetDifficulty());
#else
                InstancePlayerBind* pBind = bot->GetBoundInstance(area->map_id, bot->GetDifficulty(mapEntry->IsRaid()));
#endif
...
void PlayerbotAI::UpdateAIInternal(uint32 elapsed, bool minimal)
{
    ...
    if (bot->GetSession()->IsLogingOut())
    {
        ...
        if (!master || (IsSafe(master) && master->GetSession()->IsLogingOut()))
            logout = true;
        ...
        if (master && IsSafe(master) && (master->HasFlag(PLAYER_FLAGS, PLAYER_FLAGS_RESTING) || master->IsTaxiFlying() ||
        ...
        {
            if (master && IsSafe(master) && master->GetPlayerbotMgr())
            {
...
void PlayerbotAI::HandleTeleportAck()
{
	...
	if (bot->IsBeingTeleportedNear())
	{
       ...
       /* WorldLocation dest = bot->GetTeleportDest();
        bot->Relocate(dest.x, dest.y, dest.z, dest.o);*/
...
void PlayerbotAI::HandleBotOutgoingPacket(const WorldPacket& packet)
{
    ...
    case SMSG_MOVE_KNOCK_BACK: // handle knockbacks
    {
        ...
        if (!dest_calculated)
        {
            sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Knockback fail to calculate");
            return;
        }
        ...
        WorldPackets::Movement::MoveKnockBackAck knockAck;
        knockAck.guid = bot->GetObjectGuid();
        knockAck.movementInfo = bot->m_movementInfo;
        bot->GetSession()->HandleMoveKnockBackAck(knockAck);
        ...
        sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "%s: KNOCKBACK x: %f, y: %f, z: %f, time: %f, dist: %f, maxHeight: %f inPlace: %u, landTime: %u", bot->GetName(), dest_calculated.getX(), dest_calculated.getY(), dest_calculated.getZ(), timeToLand, distToLand, maxHeight, jumpInPlace, jumpTime);
        return;
    }
	default:
		botOutgoingPacketHandlers.AddPacket(packet);
...
void PlayerbotAI::ChangeEngine(BotState type)
{
    ...
    if (currentEngine != engine)
    {
        ...
        case BotState::BOT_STATE_COMBAT:
            sLog.Out(LOG_BASIC, LOG_LVL_DEBUG,  "=== %s COMBAT ===", bot->GetName());
            break;
        case BotState::BOT_STATE_NON_COMBAT:
            sLog.Out(LOG_BASIC, LOG_LVL_DEBUG,  "=== %s NON-COMBAT ===", bot->GetName());
            break;
        case BotState::BOT_STATE_DEAD:
            sLog.Out(LOG_BASIC, LOG_LVL_DEBUG,  "=== %s DEAD ===", bot->GetName());
            break;
        case BotState::BOT_STATE_REACTION:
            sLog.Out(LOG_BASIC, LOG_LVL_DEBUG, "=== %s REACTION ===", bot->GetName());
            break;
        default: break;
        }
    }
...
{
    if (!bot->IsInWorld() || bot->IsBeingTeleported() || (GetMaster() && IsSafe(GetMaster()) && GetMaster()->IsBeingTeleported()))
    {
    ...
    if (master && IsSafe(master) && master != bot && !HasActivePlayerMaster() && (!group || group->GetLeaderGuid() != master->GetObjectGuid()))
    {
    ...
    if ((!master || !HasActivePlayerMaster()) && group && !IsRealPlayer())
    {
        ...
        if (!newMaster || newMaster->GetPlayerbotAI())
            for (GroupReference* gref = group->GetFirstMember(); gref; gref = gref->next())
            {
                ...
                if (!member->IsInWorld() || !IsSafe(member))
                    continue;
    ...
    if (master && IsSafe(master) && master->IsInWorld())
	{
...
std::list<std::string> PlayerbotAI::GetStrategies(BotState type)
{
...
bool PlayerbotAI::PlaySound(uint32 emote)
{
    if (false)
    {
        // PlayDistanceSound not available in vmangos
        return true;
    }
...
{
    WorldPackets::Misc::TextEmote pkt;
    pkt.textEmote = emote;
    pkt.emoteNum = urand(0, EmoteAction::GetNumberOfEmoteVariants((TextEmotes)emote, bot->GetRace(), bot->GetGender()) - 1);
    pkt.guid = ((master && IsSafe(master) && (sServerFacade.GetDistance2d(bot, master) < 30.0f) && urand(0, 1)) ? master->GetObjectGuid() : (bot->GetSelectionGuid() && urand(0, 1)) ? bot->GetSelectionGuid() : ObjectGuid());
    bot->GetSession()->HandleTextEmoteOpcode(pkt);
...
ChatChannelSource PlayerbotAI::GetChatChannelSource(Player* bot, uint32 type, std::string channelName)
{
    if (type == CHAT_MSG_CHANNEL)
    {
        ...
        else
        {
            ...
            const Channel* channel = cMgr->GetChannel(channelName, bot->GetSession()->GetPlayerPointer());
...
bool PlayerbotAI::SayToGuild(std::string msg, bool likePlayer)
{
    ...
    if (bot->GetGuildId())
    {
        if (Guild* guild = sGuildMgr.GetGuildById(bot->GetGuildId()))
        {
            ...
            for (auto& player : sRandomPlayerbotMgr.GetPlayersSnapshot())
            {
                if (player.second->GetGuildId() == bot->GetGuildId())
                {
                        ...
                        sPlayerbotAIConfig.llmBotToBotChatChance))
                    {
                        ...
                        // QueuePacket not supported in vmangos
                        return false;
                    }
                    break;
                }
...
bool PlayerbotAI::SayToWorld(std::string msg)
{
    ...
    //no zone
    if (Channel* worldChannel = cMgr->GetChannel("World", bot->GetSession()->GetPlayerPointer()))
    {
        worldChannel->Say(bot->GetObjectGuid(), msg.c_str(), LANG_UNIVERSAL);
        return true;
    }
...
bool PlayerbotAI::SayToGeneral(std::string msg)
{
    ...
    for (auto const& [key, channel] : cMgr->GetChannels())
    {
        ...
        if (channel && channel->GetChannelId() == 1
            && (channel->GetName().find(GetLocalizedAreaName(current_zone)) != std::string::npos))
        {
            channel->Say(bot->GetObjectGuid(), msg.c_str(), LANG_UNIVERSAL);
            return true;
        }
    }
...
bool PlayerbotAI::SayToTrade(std::string msg)
{
    ...
    for (auto const& [key, channel] : cMgr->GetChannels())
    {
        if (channel && channel->GetChannelId() == 2
            && (channel->GetName().find(GetLocalizedAreaName(GetAreaEntryByAreaID(ImportantAreaId::CITY))) != std::string::npos))
        {
            channel->Say(bot->GetObjectGuid(), msg.c_str(), LANG_UNIVERSAL);
            return true;
        }
...
bool PlayerbotAI::SayToLFG(std::string msg)
{
    ...
    {
        //check for current zone
        if (channel && channel->GetChannelId() == 26)
        {
            channel->Say(bot->GetObjectGuid(), msg.c_str(), LANG_UNIVERSAL);
            return true;
        }
...
bool PlayerbotAI::SayToLocalDefense(std::string msg)
{
    ...
    {
        //check for current zone
        if (channel && channel->GetChannelId() == 22
            && (channel->GetName().find(GetLocalizedAreaName(current_zone)) != std::string::npos))
        {
            channel->Say(bot->GetObjectGuid(), msg.c_str(), LANG_UNIVERSAL);
            return true;
...
bool PlayerbotAI::SayToWorldDefense(std::string msg)
{
    ...
    for (auto const& [key, channel] : cMgr->GetChannels())
    {
        if (channel && channel->GetChannelId() == 23)
        {
            channel->Say(bot->GetObjectGuid(), msg.c_str(), LANG_UNIVERSAL);
            return true;
        }
...
bool PlayerbotAI::SayToGuildRecruitment(std::string msg)
{
    ...
    for (auto const& [key, channel] : cMgr->GetChannels())
    {
        ...
        if (channel && channel->GetChannelId() == 25
            && (channel->GetName().find(GetLocalizedAreaName(GetAreaEntryByAreaID(ImportantAreaId::CITY))) != std::string::npos))
        {
            channel->Say(bot->GetObjectGuid(), msg.c_str(), LANG_UNIVERSAL);
            return true;
...
bool PlayerbotAI::Whisper(std::string msg, std::string receiverName, bool likePlayer)
{
    ...
    {
        Language whisperLang = (bot->GetTeam() == ALLIANCE) ? LANG_COMMON : LANG_ORCISH;
        WorldPacket data;
        ChatHandler::BuildChatPacket(data, CHAT_MSG_WHISPER, msg.c_str(), whisperLang, bot->GetChatTag(), bot->GetObjectGuid(), bot->GetName());
        rPlayer->GetSession()->SendPacket(&data);
    }
...
bool PlayerbotAI::HasAura(std::string name, Unit* unit, bool maxStack, bool checkIsOwner, int maxAuraAmount, bool hasMyAura, int minDuration, int auraTypeId)
{
    ...
    if (ids.empty())
    {
        //sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "Please add %s to spell list", name.c_str());
    ...
    if(spellId)
    {
        ...
        if (spellInfo)
        {
            ...
            if(spellRangeEntry)
            {
                ...
                if (spellRangeEntry->maxRange <= 5.0f)
                {
                    rangeMod = bot->GetCombatReach(true);
                }
                else
                {
                    ...
                    if (spellRangeEntry->maxRange > 5.0f)
                    {
                        meleeRange = bot->GetCombatReach(true);
                    }
...
bool PlayerbotAI::CastPetSpell(uint32 spellId, Unit* target)
{
    ...
    if (pet && spellId && pet->HasSpell(spellId))
    {
        ...
        if (false /* IsAutocastable not in vmangos */)
        {
        ...
        { WorldPackets::Pet::PetAction petPkt; petPkt.ReadFromWorldPacket(data); bot->GetSession()->HandlePetAction(petPkt); }
        return true;
    }
...
bool PlayerbotAI::CanCastVehicleSpell(uint32 spellId, Unit* target)
{
    ...
    uint32 CastingTime = !(spellInfo->HasAttribute(SPELL_ATTR_EX_IS_CHANNELED)) ? GetSpellCastTime(spellInfo, vehicle) : spellInfo->GetDuration();
    ...
    if (siegePos.isSet())
        ...
        dest = WorldLocation(spellTarget->GetMapId(), spellTarget->GetPositionX(), spellTarget->GetPositionY(), spellTarget->GetPositionZ(), spellTarget->GetOrientation());
...
void PlayerbotAI::InterruptSpell(bool withMeleeAndAuto)
{
    for (int type = CURRENT_MELEE_SPELL; type < CURRENT_CHANNELED_SPELL; type++)
    {
        ...
        if (currentSpell && (currentSpell->getState() == SPELL_STATE_CASTING))
        {
...
{
    const SpellEntry* spellEntry = sSpellMgr.GetSpellEntry(spellId);
    if (spellEntry)
    {
        ...
        {
            if (true /* CanNoReagentCast not in vmangos */)
            {
...
ActivePiorityType PlayerbotAI::GetPriorityType()
{
    ...
    if (group)
    {
        for (GroupReference* gref = group->GetFirstMember(); gref; gref = gref->next())
        {
            ...
            if (!member || !member->IsInWorld() || !IsSafe(member))
                continue;
...
// ChatHandler command stubs are registered in OldBotCommandStubs.cpp for vmangos
...
{
    // vmangos loot system is different - WorldObject doesn't have m_loot
    return false;
}
    ...
    if (guid.IsCreature())
    {
        ...
        if (creature && sServerFacade.GetDeathState(creature) == CORPSE)
        {
            ...
            if (creature->loot.unlootedCount > 0)
                /* loot iteration not available in vmangos */;
...
void PlayerbotAI::AccelerateRespawn(Creature* creature, float accelMod)
{
    ...
    {
        CreatureData const* data = sObjectMgr.GetCreatureData(creature->GetDBTableGUIDLow());

        if (!data)
            return;

        m_respawnDelay = data->GetRandomRespawnTime() * IN_MILLISECONDS;
        m_corpseAccelerationDecayDelay = 60000 /* MINIMUM_LOOTING_TIME */;
        ...
        if (m_respawnDelay < totalDelay * accelMod)
        {
            if ((m_corpseAccelerationDecayDelay- ((totalDelay * accelMod) - m_respawnDelay)) < 0)
                sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "m_corpseAccelerationDecayDelay: %d, totalDelay: %d, accelMod: %f, m_respawnDelay: %d", m_corpseAccelerationDecayDelay, totalDelay, accelMod, m_respawnDelay);
    ...
    if (!m_corpseAccelerationDecayDelay && creature->loot.unlootedCount == 0)
    {
    ...
    {        uint32 defaultDelay = 2 * MINUTE;
        ...
        if (0 /* CorpseDelay not in vmangos */)
            defaultDelay = 0 /* CorpseDelay not in vmangos */;
        else if (false /* IsEncounter not in vmangos */)
        {
        ...
        // SetCorpseAccelerationDelay not in vmangos

        // ReduceCorpseDecayTimer not in vmangos
        return;
    }
...
Item* PlayerbotAI::FindStoneFor(Item* weapon) const
{
    ...
    {
        for (uint8 i = 0; i < (sizeof(uPriorizedSharpStoneIds)/sizeof(uPriorizedSharpStoneIds[0])); ++i)
        {
    ...
    {
        for (uint8 i = 0; i < (sizeof(uPriorizedWeightStoneIds)/sizeof(uPriorizedWeightStoneIds[0])); ++i)
        {
...
SpellEntry const* spellInfo = sSpellMgr.GetSpellEntry(spellid);
   ...
   if (!enchantid)
   {
      sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "%u: Invalid enchantid %s %s" , enchantid , " report to devs", bot->GetName());
      return;
   }
      ...
      !((1 << pItem->GetProto()->InventoryType) & spellInfo->EquippedItemInventoryTypeMask))
   {
      ...
      sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "%s: items could not be enchanted, wrong item type equipped", bot->GetName());
   ...
   sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "%s: items was enchanted successfully!", bot->GetName());
}
...
{
    // vmangos QueuePacket expects unique_ptr<const ClientPacket>, not WorldPacket
    // For movement packets, construct a MovementPacket and handle directly
    WorldPackets::Movement::MovementPacket movePkt;
    movePkt.ReadFromWorldPacket(pkt);
    bot->GetSession()->HandleMovementOpcodes(movePkt);
}
    ...
    for (SpellClickInfoMap::const_iterator itr = clickPair.first; itr != clickPair.second; ++itr)
    {
        if (itr->second.IsFitToRequirements(bot, creature))
        {
            ...
            if (itr->second.spellId)
            {
                ...
                sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "WorldSession::HandleSpellClick: npc_spell_click with entry %u has 0 in spell_id. Not handled custom case?", creature->GetEntry());
        }
    }
#endif
    return false;
