...
{    
    if (!isOverworld())
    {
        ...
        if (map)
            return map->name ? std::string(map->name) : "";
    }
    ...
    if (fullName)
    {
        ...
        while (zoneId > 0)
        {
            ...
            std::string subAreaName = parentArea->Name;
...
int32 WorldPosition::getAreaLevel() const
{
    ...
    if (AreaTableEntry const* area = GetArea())
        return sTravelMgr.GetAreaLevel(area->Id);
...
std::set<GenericTransport*> WorldPosition::getTransports(uint32 entry)
{
    ...
    for (auto transport : std::set<GenericTransport*>()) //Boats&Zeppelins.
...
bool WorldPosition::loadMapAndVMap(uint32 mapId, uint32 instanceId, int x, int y)
{
    ...
    if (!hasMmap)
    {
        ...
        else
        {
            ...
            if (!loadedMap)
                loadedMap = // loadMapInstance not in vmangos
...
std::vector<WorldPosition> WorldPosition::getPathFromPath(const std::vector<WorldPosition>& startPath, const Unit* bot, uint8 maxAttempt) const
{
    ...
    if (bot && instanceId == bot->GetInstanceId())
        ...
        pathfinder = std::make_unique<PathFinder>(bot); // vmangos PathInfo needs Unit*

// setAreaCost not in vmangos
// setAreaCost not in vmangos
// setAreaCost not in vmangos
...
{
#ifndef MANGOSBOT_TWO         
    return false /* GetReachableRandomPointOnGround not in vmangos */;
#else
...
bool WorldPosition::isUnderground() const
{
    float ground = 0.0f;
    if (getTerrain())
        getTerrain()->GetWaterLevel(x, y, z, &ground);
    return z < ground - 0.5f;
}
...
uint32 WorldPosition::getUnitsAggro(const std::list<ObjectGuid>& units, const Player* bot) const
{
    ...
    for (auto guid : units)
    {
        ...
        if (this->sqDistance(unit) > unit->GetMeleeReach() * unit->GetMeleeReach())
            continue;
