...
public:
    ...
    std::list<std::string> GetStrategies(BotState type);
...
public:
    ...
    // vmangos sets m_remoteIpAddress to "<BOT>" for bot sessions (no socket).
    ...
    //Check if player is safe to use (thread-safe with Continents.Instanciate sub-maps).
    static bool IsSafe(Player* player, WorldObject* obj) { return obj && obj->GetMapId() == player->GetMapId() && obj->GetInstanceId() == player->GetInstanceId() && player->FindMap() && obj->FindMap() == player->FindMap() && (!obj->IsPlayer() || !((Player*)obj)->IsBeingTeleported()); }
    bool IsSafe(WorldObject* obj) { return IsSafe(bot, obj); }
    bool IsSafe(Player* player) { return IsSafe(bot, (WorldObject*)player); }
    ...
    bool IsActivityAllowedCached(ActivityType activityType = ALL_ACTIVITY) const { return allowActive[activityType]; }
    ...
    bool CanEnterArea(const AreaTriggerEntry* area);
    void Unmount();
