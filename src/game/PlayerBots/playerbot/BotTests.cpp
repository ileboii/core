...
void LogAnalysis::AnalysePid()
{
    ...
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "========= LAST SERVER RUNTIME: %s", ss.str().c_str());
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "========= AVG DIFF [%d] & AVG ACTIVE BOTS [%d/%d]", aDiff, aBots, maxBots);
}
...
void LogAnalysis::AnalyseEvents()
{
    ...
    for (auto event : eventMin)
    {
        ...
        if (eventCount[event.first] < event.second)
            sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "Event %s was only seen %d times", event.first.c_str(), eventCount[event.first]);
    }
    ...
    for (auto quest : questId)
    {
        ...
        if (!questEndCount[quest.first])
        {
            if (questCompletedCount[quest.first] > 1)
                sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "Quest %s (%d) has %d objectives completed but was never turned in.", quest.first.c_str(), quest.second, questCompletedCount[quest.first]);
            else if (questObjective[quest.first] > 1)
                sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "Quest %s (%d) has %d objectives partially completed but was never turned in.", quest.first.c_str(), quest.second, questObjective[quest.first]);
            else if (questStartCount[quest.first] > 10)
                sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "Quest %s (%d) started %d times but never turned in.", quest.first.c_str(), quest.second, questStartCount[quest.first]);
        }
    }
...
void LogAnalysis::AnalyseCounts()
{
    ...
    for (auto& count : counts)
    {
        ...
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "[Top %d %s]", maxShow, countType[count.first].c_str());
        for (uint32 i = 0; i < std::min(maxShow, (uint32)list.size()); i++)
            sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "%s (%d)", list[i].first.c_str(), list[i].second);
    }
}