...
{
    sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Preparing to %s randomize...", (incremental ? "incremental" : "full"));
    Prepare();
    ...
    sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Resetting player...");
    ...
    sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Initializing skills (step 1)...");
    InitAllSkills();

    pmo = sPerformanceMonitor.start(PERF_MON_RNDBOT, "PlayerbotFactory_Talents");
    sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Initializing talents...");
    ...
    if (isRealRandomBot)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Initializing mounts...");
        InitMounts();
    }

    sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Initializing skills (step 2)...");
    UpdateTradeSkills();

    if (isRealRandomBot)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Initializing reputations...");
        InitReputations();
        
    }

    pmo = sPerformanceMonitor.start(PERF_MON_RNDBOT, "PlayerbotFactory_Equip");
    sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Initializing equipmemt...");
    if (bot->GetLevel() >= sPlayerbotAIConfig.minEnchantingBotLevel)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Initializing enchant templates...");
    ...
    if (isRandomBot)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Initializing ammo...");
        InitAmmo();

        sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Initializing food...");
        InitFood();

        sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Initializing potions...");
        InitPotions();

        sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Initializing reagents...");
        InitReagents();
    }

    if (!incremental)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Initializing consumables...");
        AddConsumables();
    }
    ...
    if (!incremental && isRandomBot)
    {
        ...
        sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Initializing second equipment set...");
        InitSecondEquipmentSet();
        if (pmo) pmo->finish();*/

        sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Initializing inventory...");
        InitInventory();
    }
    ...
    if (isRandomBot)
    {
        ...
        sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Initializing guilds & ArenaTeams");
        InitGuild();
#ifndef MANGOSBOT_ZERO
        if (bot->GetLevel() >= 70)
            InitArenaTeam();
...
void PlayerbotFactory::InitPet()
{
    ...
    if (!pet)
    {
        ...
        if (ids.empty())
        {
            sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "No pets available for bot %s (%d level)", bot->GetName(), bot->GetLevel());
            return;
        }
		...
		for (int i = 0; i < 100; i++)
		{
            ...
            // CastOwnerTalentAuras not in vmangos
            pet->UpdateAllStats();
            bot->SetPet(pet);
            ...
            sLog.Out(LOG_BASIC, LOG_LVL_DEBUG,   "Bot %s: assign pet %d (%d level)", bot->GetName(), co->entry, bot->GetLevel());
    ...
    {
        sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "Cannot create pet for bot %s", bot->GetName());
        return;
    }
...
void PlayerbotFactory::InitPetSpells()
{
    ...
    if (bot->GetClass() == CLASS_HUNTER)
    {
        ...
        {
            pet->AddSpell(naturalArmorSpellId);
        }
...
void PlayerbotFactory::ResetQuests()
{
    ...
    for (ObjectMgr::QuestMap::const_iterator i = questTemplates.begin(); i != questTemplates.end(); ++i)
    {
        ...
        {
            uint32 quest = bot->GetUInt32Value(PLAYER_QUEST_LOG_1_1 + slot * MAX_QUEST_OFFSET + QUEST_ID_OFFSET);
            if (quest == entry)
            {
        ...
        /* getQuestStatusMap not in vmangos */;
        ///* getQuestStatusMap not in vmangos */;
        //*(QuestStatusData*)nullptr /* getQuestStatusMap not in vmangos */.m_status = QUEST_STATUS_NONE;
    }
    ...
    for(uint8 slot = 0; slot < EQUIPMENT_SLOT_END; ++slot)
    {
        ...
        do
        {
            ...
            if (slot == EQUIPMENT_SLOT_BODY || slot == EQUIPMENT_SLOT_TABARD)
            {
                ...
                sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Bot #%d %s:%d <%s>: %u possible items for slot %d", bot->GetGUIDLow(), bot->GetTeam() == ALLIANCE ? "A" : "H", bot->GetLevel(), bot->GetName(), uint32(ids.size()), slot);
            ...
            else
            {
                ...
                for (uint32 index = 0; index < ids.size(); ++index)
                {
                    ...
                    if (found)
                    {
                        ...
                        {
                            if (oldItem)
                                sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Bot #%d %s:%d <%s>: Old Item: slot: %u, id: %u, value: %u (%s)", bot->GetGUIDLow(), bot->GetTeam() == ALLIANCE ? "A" : "H", bot->GetLevel(), bot->GetName(), slot, oldProto->ItemId, oldStatValue, oldProto->Name1);
                            sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Bot #%d %s:%d <%s>: New Item: slot: %u, id: %u, value: %u (%s)", bot->GetGUIDLow(), bot->GetTeam() == ALLIANCE ? "A" : "H", bot->GetLevel(), bot->GetName(), slot, proto->ItemId, newStatValue, proto->Name1);
                        }
                        break;
                    }
                }
        ...
        if (!found)
        {
            if (slot != EQUIPMENT_SLOT_TRINKET1 && slot != EQUIPMENT_SLOT_TRINKET2)
                sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Bot #%d %s:%d <%s>: no items for slot %d, quality >= %u", bot->GetGUIDLow(), bot->GetTeam() == ALLIANCE ? "A" : "H", bot->GetLevel(), bot->GetName(), slot, quality);
            continue;
        }
    }

    /*if (incremental && oldGS != newGS)
        sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Bot #%d %s:%d <%s>: GS: %u -> %u", bot->GetGUIDLow(), bot->GetTeam() == ALLIANCE ? "A" : "H", bot->GetLevel(), bot->GetName(), oldGS, newGS);*/
...
void PlayerbotFactory::InitSecondEquipmentSet()
{
    ...
    for (std::map<uint32, std::vector<uint32> >::iterator i = items.begin(); i != items.end(); ++i)
    {
        ...
        if (ids.empty())
        {
            sLog.Out(LOG_BASIC, LOG_LVL_DEBUG,   "%s: no items to make second equipment set for slot %d", bot->GetName(), i->first);
            continue;
        }
...
void PlayerbotFactory::InitTradeSkills()
{
    ...
    for (uint32 id = 0; id < sCreatureStorage.GetMaxEntry(); ++id)
    {
        ...
        for (TrainerSpellMap::const_iterator itr = trainer_spells->spellList.begin(); itr != trainer_spells->spellList.end(); ++itr)
        {
            ...
            uint32 reqLevel = 0;
            reqLevel = std::max(reqLevel, tSpell->reqLevel);
...
#else
            if (!tSpell->spell.empty())
            {
                for (auto learnSpell : tSpell->spell)
                {
                    bool learned = false;
                    for (int j = 0; j < 3; ++j)
                    {
...
void PlayerbotFactory::InitTalents(uint32 specNo)
{
    ...
    for (std::map<uint32, std::vector<TalentEntry const*> >::iterator i = spells.begin(); i != spells.end(); ++i)
    {
        ...
        if (spells.empty())
        {
            sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "%s: No spells for talent row %d", bot->GetName(), i->first);
            continue;
        }
...
void PlayerbotFactory::AddPrevQuests(uint32 questId, std::list<uint32>& questIds)
{
    ...
    for (std::vector<int32>::const_iterator iter = quest->prevQuests.begin(); iter != quest->prevQuests.end(); ++iter)
    {
...
void PlayerbotFactory::InitQuests(std::list<uint32>& questMap)
{
    ...
    for (std::list<uint32>::iterator i = questMap.begin(); i != questMap.end(); ++i)
    {
        ...
        sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Bot %s (%d level) rewarded quest %d: %s (MinLevel=%d, QuestLevel=%d)",
...
void PlayerbotFactory::InitMounts()
{
    ...
    for (uint32 type = 0; type < 4; type++)
    {
        ...
        if (spell)
        {
            ...
            sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Bot %d (%d) learned %s mount %d", bot->GetGUIDLow(), bot->GetLevel(), type == 0 ? "slow" : (type == 1 ? "fast" : "flying"), spell);
        }
    }
}
...
void PlayerbotFactory::InitPotions()
{
    ...
    for (int i = 0; i < 2; ++i)
    {
        ...
        if (!itemId)
        {
            sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "No potions (type %d) available for bot %s (%d level)", effect, bot->GetName(), bot->GetLevel());
            continue;
        }
...
void PlayerbotFactory::InitFood()
{
    ...
    for (int i = 0; i < 2; ++i)
    {
        ...
        if (!itemId)
        {
            sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "No food (category %d) available for bot %s (%d level)", category, bot->GetName(), bot->GetLevel());
            continue;
        }
        ItemPrototype const* proto = sObjectMgr.GetItemPrototype(itemId);
        if (!proto) continue;
...
void PlayerbotFactory::InitReagents()
{
    ...
    for (std::list<uint32>::iterator i = items.begin(); i != items.end(); ++i)
    {
        ...
        if (!proto)
        {
            sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "No reagent (ItemId %d) found for bot %d (Class:%d)", *i, bot->GetGUIDLow(), bot->GetClass());
            continue;
        }
        ...
        sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Bot %d got reagent %s x%d", bot->GetGUIDLow(), proto->Name1, randCount);
    }
    ...
    for (PlayerSpellMap::iterator itr = bot->GetSpellMap().begin(); itr != bot->GetSpellMap().end(); ++itr)
    {
        ...
        for (const auto& totem : pSpellInfo->Totem)
        {
            if (totem && !bot->HasItemCount(totem, 1))
            {
                ...
                if (!proto)
                {
                    sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "No totem (ItemId %d) found for bot %d (Class:%d)", totem, bot->GetGUIDLow(), bot->GetClass());
                    continue;
                }

                Item* newItem = bot->StoreNewItemInInventorySlot(totem, 1);

                sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Bot %d got totem %s x%d", bot->GetGUIDLow(), proto->Name1, 1);
            }
        }
#ifndef MANGOSBOT_ZERO
        for (const auto& totemCat : pSpellInfo->TotemCategory)
        {
            if (totemCat && !bot->HasItemTotemCategory(totemCat))
            {
                ...
                if (itemId)
                {
                    ...
                    if (!proto)
                    {
                        sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "No totem (ItemId %d) found for bot %d (Class:%d)", itemId, bot->GetGUIDLow(), bot->GetClass());
                        continue;
                    }
...
void PlayerbotFactory::InitInventoryTrade()
{
    ...
    if (!itemId)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "No trade items available for bot %s (%d level)", bot->GetName(), bot->GetLevel());
        return;
    }
...
void PlayerbotFactory::InitGuild()
{
    ...
    if (guilds.empty())
    {
        sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "No random guilds available");
        return;
    }
    ...
    if (!guild)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "Can't join random guild, ID: %u", guildId);
        return;
    }
    ...
    if ((num && guild->GetMemberSize() < num) || (!num && guild->GetMemberSize() < urand(10, 15)))
    {
        ...
        sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Bot #%d %s:%d <%s>: Guild <%s> R: %s", bot->GetGUIDLow(), bot->GetTeam() == ALLIANCE ? "A" : "H", bot->GetLevel(), bot->GetName(), guild->GetName().c_str(), guild->GetRankName(rankId).c_str());
    }
