...
void PlayerbotHelpMgr::LoadStrategies(std::string className, AiObjectContext* context)
{
    ...
    for (auto strategyName : context->GetSupportedStrategies())
    {
        ...
        if (!strategy)
            sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "Strategy %s is defined but can not be created.", strategyName.c_str());
        ...
        for (auto stat : states)
        {
            ...
            if (!triggers.empty())
            {
                ...
                for (auto& triggerNode : triggers)
                {
                    ...
                    if (!trigger)
                        sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "Trigger %s is used in strategy %s but can not be created.", triggerNode->getName().c_str(), strategyName.c_str());
                    ...
                    if (trigger)
                    {
                        ...
                        for (auto nextAction : nextActionList)
                        {
                            ...
                            if (!action)
                                sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "Action %s is defined in strategy %s for trigger %s but can not be created.", nextAction->getName().c_str(), strategyName.c_str(), triggerNode->getName().c_str());
            ...
            if (strategy->getDefaultActions(state))
            {
                ...
                for (auto nextAction : nextActionList)
                {
                    ...
                    if (!action)
                        sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "Default action %s is defined in strategy %s but can not be created.", action->getName().c_str(), strategyName.c_str());
...
void PlayerbotHelpMgr::PrintCoverage()
{
    for (auto typeCov : coverageMap)
    {
        ...
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "%s help coverage %d/%d : %d%%", typeCov.first, hasNames, totalNames, (hasNames * 100) / totalNames);

        if (missingNames.empty())
            continue;

        sLog.Out(LOG_BASIC, LOG_LVL_BASIC, "Missing:");

        std::sort(missingNames.begin(), missingNames.end());
        for (auto name : missingNames)
            sLog.Out(LOG_BASIC, LOG_LVL_BASIC, "%s", name.c_str());
    }
}
...
void PlayerbotHelpMgr::FormatHelpTopics()
{
    for (auto& helpText : botHelpText)
    {
        ...
        {
            FormatHelpTopic(helpText.second.m_text_locales[i]);
        }
    }
}
...
{
    sLog.Out(LOG_BASIC, LOG_LVL_BASIC, "Loading playerbot texts...");
    ...
    if (results)
    {
        do
        {
            ...
            for (uint8 i = 1; i < MAX_LOCALE; ++i)
            {
                text_locale[i] = fields[i + 2].GetString();
            }
    ...
    sLog.Out(LOG_BASIC, LOG_LVL_BASIC, "%d playerbot helptexts loaded", count);
...
std::string PlayerbotHelpMgr::GetBotText(std::string name)
{
    ...
    {
        sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "Can't get bot help text %s! No bots help texts loaded!", name.c_str());
        return "";
    }
    if (botHelpText.find(name) == botHelpText.end())
    {
        sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Can't get bot help text %s! No bots help texts for this name!", name.c_str());
        return "";
    }
