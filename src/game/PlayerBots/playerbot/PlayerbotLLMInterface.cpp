...
inline void SetNonBlockingSocket(int sock) {
    ...
    if (ioctlsocket(sock, FIONBIO, &mode) != 0) {
        sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "BotLLM: Failed to set non-blocking mode on socket. Error: %d", WSAGetLastError());
    }
#else
    int flags = fcntl(sock, F_GETFL, 0);
    if (flags == -1 || fcntl(sock, F_SETFL, flags | O_NONBLOCK) == -1) {
        sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "BotLLM: Failed to set non-blocking mode on socket. Error: %s", strerror(errno));
    }
#endif
}
...
#ifdef _WIN32
                sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "BotLLM: recv error: %d", WSAGetLastError());
#else
                sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "BotLLM: recv error: %s", strerror(errno));
#endif
                break;
            }
            }
...
std::string PlayerbotLLMInterface::Generate(const std::string& prompt, int timeOutSeconds, int maxGenerations, std::vector<std::string> & debugLines) {
    ...
    if (WSAStartup(MAKEWORD(2, 2), &wsaData) != 0) {
        ...
        sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "BotLLM: WSAStartup failed");
        sPlayerbotLLMInterface.generationCount--;
        return "error";
    }
#endif
    ...
    if (getaddrinfo(parsedUrl.hostname.c_str(), portStr.c_str(), &hints, &res) != 0) {
        ...
        sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "BotLLM: Failed to resolve hostname: %s", parsedUrl.hostname.c_str());
#ifdef _WIN32
        WSACleanup();
#endif
        sPlayerbotLLMInterface.generationCount--;
    ...
    if (sock == INVALID_SOCKET) {
        ...
        sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "BotLLM: Socket creation failed. Error: %d", WSAGetLastError());
        WSACleanup();
        freeaddrinfo(res);
        sPlayerbotLLMInterface.generationCount--;
        return "error";
    ...
    if (sock < 0) {
        ...
        sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "BotLLM: Socket creation failed. Error: %s", strerror(errno));
        freeaddrinfo(res);
        sPlayerbotLLMInterface.generationCount--;
        return "error";
    }
...
#ifdef _WIN32
        sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "BotLLM: Connection to server failed. Error: %d", WSAGetLastError());
        closesocket(sock);
        WSACleanup();
#else
        sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "BotLLM: Connection to server failed. Error: %s", strerror(errno));
        close(sock);
#endif
        freeaddrinfo(res);
        sPlayerbotLLMInterface.generationCount--;
    ...
    if (parsedUrl.https)
    {
        ...
        if (!ctx) {
            ...
            sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "BotLLM: Failed to create SSL context: %s", GetSSLError().c_str());
#ifdef _WIN32
            closesocket(sock);
            WSACleanup();
#else
        ...
        if (!ssl) {
            ...
            sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "BotLLM: Failed to create SSL object: %s", GetSSLError().c_str());
            SSL_CTX_free(ctx);
#ifdef _WIN32
            closesocket(sock);
            WSACleanup();
        ...
        if (ssl_connect_result <= 0) {
            ...
            sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "BotLLM: %s", error_msg.c_str());
    ...
    if (parsedUrl.https && ssl) {
        ...
        if (write_result < 0) {
            ...
            sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "BotLLM: Failed to send request");
        }
    }
    ...
    if (bytesRead == SOCKET_ERROR) {
        ...
        sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "BotLLM: Error reading response. Error: %d", WSAGetLastError());
    }
    closesocket(sock);
    WSACleanup();
#else
    if (bytesRead < 0) {
        if (debug)
            debugLines.push_back("Error reading response");
        sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "BotLLM: Error reading response. Error: %s", strerror(errno));
    }
    close(sock);
#endif
