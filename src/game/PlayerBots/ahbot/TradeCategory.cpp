...
bool TradeSkill::ContainsInternal(ItemPrototype const* proto)
{
    ...
    {
        ItemPrototype const* recipe = sObjectMgr.GetItemPrototype(itemId);
        if (!recipe)
            continue;
...
bool TradeSkill::IsCraftedBySpell(ItemPrototype const* proto, SpellEntry const *entry)
{
    if (reagent)
    {
        for (uint32 x = 0; x < MAX_SPELL_REAGENTS; ++x)
        {
            ...
            {
                sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "%s is a reagent for %s", proto->Name1, entry->SpellName[0]);
                return true;
            }
        }
    }
    else
    {
        for (int i = 0; i < 3; ++i)
        {
            if (entry->Effect[i] == SPELL_EFFECT_CREATE_ITEM)
            {
                ...
                {
                    sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "%s is crafted by %s", proto->Name1, entry->SpellName[0]);
                    return true;
                }
            }
        }
