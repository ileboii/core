...
void AhBot::Init()
{
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "Initializing AhBot by ike3");
...
{
    std::thread t([]() { auctionbot.ForceUpdate(); });
    t.detach();
}
...
void AhBot::ForceUpdate()
{
	...
	if (!allBidders.size())
	{
		sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "Ahbot is enabled but there are no bidders available");
		return;
	}
	...
	sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "AhBot auction check finished. %d auctions answered, %d new auctions added. Next check in %d seconds",
        ...
        Category*& category, int& auction)
{
            ...
            itr != auctionEntryMap.end(); ++itr)
    {
        ...
        {
            sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "%s (x%d) in auction %d: price cannot be determined",
...
int AhBot::Answer(int auction, Category* category, ItemBag* inAuctionItems)
{
    ...
    for (std::vector<AuctionEntry*>::iterator itr = entries.begin(); itr != entries.end(); ++itr)
    {
        ...
        {
            sLog.Out(LOG_BASIC, LOG_LVL_DETAIL,  "%s (x%d) in auction %d: unavailable item",
                    item->GetProto()->Name1, item->GetCount(), auctionIds[auction]);
            continue;
        }
        ...
        {
            sLog.Out(LOG_BASIC, LOG_LVL_DETAIL,  "%s (x%d) in auction %d: answer count %d > %d (max)",
        ...
        {
            sLog.Out(LOG_BASIC, LOG_LVL_DETAIL,  "%s (x%d) in auction %d: above max required or item level",
                    item->GetProto()->Name1, item->GetCount(), auctionIds[auction]);
            continue;
        }
        ...
        if (!price)
        {
            sLog.Out(LOG_BASIC, LOG_LVL_DETAIL,  "%s (x%d) in auction %d: cannot determine price",
                    item->GetProto()->Name1, item->GetCount(), auctionIds[auction]);
            continue;
        }
        ...
        {
            sLog.Out(LOG_BASIC, LOG_LVL_DETAIL,  "%s (x%d) in auction %d: price %d > %d (buyout price)",
        ...
        {
            sLog.Out(LOG_BASIC, LOG_LVL_DETAIL,  "%s (x%d) in auction %d: price %d > %ld (available money)",
        ...
        {
            sLog.Out(LOG_BASIC, LOG_LVL_DETAIL,  "%s (x%d) in auction %d: %d (bid) > %d (minBid)",
        ...
        {
            sLog.Out(LOG_BASIC, LOG_LVL_DETAIL,  "%s (x%d) in auction %d: %d (startbid) > %d (minBid)",
        ...
        if (time(0) < buytime)
        {
            sLog.Out(LOG_BASIC, LOG_LVL_DETAIL,  "%s (x%d) in auction %d: will buy/bid in %ld seconds",
        ...
        {
            sLog.Out(LOG_BASIC, LOG_LVL_DETAIL,  "AhBot %d won %s (x%d) in auction %d for %d",
                    bidder, item->GetProto()->Name1, item->GetCount(), auctionIds[auction], entry->buyout);

            entry->bid = entry->buyout;
            /* entry->AuctionBidWinning(NULL); // vmangos: no AuctionBidWinning */
        }
        else
        {
            sLog.Out(LOG_BASIC, LOG_LVL_DETAIL,  "AhBot %d placed bid %d for %s (x%d) in auction %d",
...
int AhBot::AddAuctions(int auction, Category* category, ItemBag* inAuctionItems)
{
    ...
    for (int32 i = 0; i <= maxAllowedAuctionCount && available.size() > 0 && inAuctionItems->GetCount(category) < maxAllowedAuctionCount; ++i)
    {
        ...
        {
            sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "%s in auction %d: has reached max %d/%d",
        ...
        {
            ladded += 1;
            sLog.Out(LOG_BASIC, LOG_LVL_DETAIL,  "%s in auction %d: will add in %ld seconds",
                    proto->Name1, auctionIds[auction], sellTime - time(0));
            continue;
        }
        else if (time(0) - sellTime > sAhBotConfig.maxSellInterval)
        {
            sLog.Out(LOG_BASIC, LOG_LVL_DETAIL,  "%s in auction %d: too old (%ld secs)",
    ...
    sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "%s has %d upcomming and %d new auctions.",
...
int AhBot::AddAuction(int auction, Category* category, ItemPrototype const* proto)
{
    ...
    // auctionEntry->itemCount = item->GetCount(); // vmangos: no itemCount field
    // auctionEntry->itemRandomPropertyId = item->GetItemRandomPropertyId(); // vmangos: no itemRandomPropertyId field
...
void AhBot::HandleCommand(std::string command)
{
    ...
    if (!itemId)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ahbot stats - show short summary");
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ahbot expire - expire all auctions");
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ahbot update - update all auctions");
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ahbot <itemId> - show item price");
        return;
    }
    ...
    for (int i=0; i<CategoryList::instance.size(); i++)
    {
        ...
        if (category->Contains(proto))
        {
            ...
            sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "%s",out.str().c_str());
        }
    }
}
...
void AhBot::Expire(int auction)
{
    ...
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "%d auctions marked as expired in auction %d", count, auctionIds[auction]);
}
...
void AhBot::PrintStats(int auction)
{
    ...
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "%lu auctions available on auction house %d", auctions.size(), auctionIds[auction]);
}
...
void AhBot::AddToHistory(AuctionEntry* entry, uint32 won)
{
    ...
    sLog.Out(LOG_BASIC, LOG_LVL_DETAIL,  "AddToHistory: market price adjust");
    int count = 1;
...
void AhBot::LoadRandomBots()
{
    ...
    sLog.Out(LOG_BASIC, LOG_LVL_DETAIL,  "{A=%zu,H=%zu,N=%zu} bidders loaded", bidders[1].size(), bidders[2].size(), bidders[3].size());
}
...
void AhBot::CleanupPropositions()
{
    ...
    do
    {
        ...
        // if (player) player->RemoveMail(id); // vmangos: no RemoveMail on Player
        count++;
    } while (result->NextRow());
    ...
    if (count > 0)
    {
        ...
        sLog.Out(LOG_BASIC, LOG_LVL_BASIC, "%d old AH propositions removed", count);
    }
}
