...
void ItemBag::Init(bool silent)
{
    ...
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "Loading/Scanning %s...", GetName().c_str());
    ...
    for (int i = 0; i < CategoryList::instance.size(); i++)
    {
        ...
        sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "loaded %zu %s items", content[category].size(), category->GetDisplayName().c_str());
		ahcounter += content[category].size();
    }
	sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "Loaded %u items", ahcounter);
}
...
bool ItemBag::Add(ItemPrototype const* proto)
{
    ...
    if (!contains)
        sLog.Out(LOG_BASIC, LOG_LVL_DETAIL, "Item %s does not included in any category", proto->Name1);
