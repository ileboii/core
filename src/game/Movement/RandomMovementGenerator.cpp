...
void RandomMovementGenerator::_setRandomLocation(Creature &creature)
{
    if (creature.CanFly())
    {
        ...
        for (uint32 i = 0; i <= nbCyclesPerPacket * ptsPerCycle; ++i)
            path.emplace_back(static_cast<float>(i_startPosition.x + i_wanderDistance * cos(i * 2 * M_PI / ptsPerCycle)), static_cast<float>(i_startPosition.y + i_wanderDistance * sin(i * 2 * M_PI / ptsPerCycle)), i_startPosition.z);
