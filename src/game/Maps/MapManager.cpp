...
uint32 MapManager::GetContinentInstanceId(uint32 mapId, float x, float y, bool* transitionArea, uint32 zoneId)
{
    ...
    switch (mapId)
    {
            ...
            if (zoneId)
            {
                ...
                case 47:
                    if (baseInstanceId == MAP0_MIDDLE_NORTH)
                        return MAP0_ARATHI_HINTERLANDS;
                    break;

                case 3: // Badlands
                case 51: // Searing Gorge
                case 25: // Blackrock Mountain
                    if (baseInstanceId == MAP0_MIDDLE)
                        return MAP0_BADLANDS_SEARING;
                    break;
        ...
        case MAP_KALIMDOR:
        {
            ...
            if (baseInstanceId == MAP1_NORTH && zoneId)
            {
                switch (zoneId)
                {
                // North-central Kalimdor
                case 331: // Ashenvale
